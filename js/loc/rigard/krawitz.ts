...
KrawitzScenes.GuardDex = (entity: EncType, num?: number) => {
	num = num || 1;
	const dex = (entity === EncType.Guard) ? 10 : 15;
	return num * dex;
};
KrawitzScenes.GuardAtk = (entity: EncType, num?: number) => {
	num = num || 1;
	const str = (entity === EncType.Guard) ? 40 : 20;
	return num * str;
};
KrawitzScenes.GuardCha = (entity: EncType, num?: number) => {
	num = num || 1;
	let cha = (entity === EncType.Guard) ? 15 : 10;
	if (entity === EncType.Guard) {
		if (KrawitzScenes.stat.HasServantClothes) { cha /= 2; }
	} else { // Servants
		if (KrawitzScenes.stat.IsServant) { cha /= 4; } else if (KrawitzScenes.stat.HasServantClothes) { cha *= 1.5; }
	}

	return num * cha;
};
KrawitzScenes.ServantSuspicion = () => {
	if (KrawitzScenes.stat.IsServant) { return 1; } else if (KrawitzScenes.stat.HasServantClothes) { return 3; } else { return 2; }
};
KrawitzScenes.GuardSuspicion = () => {
	if (KrawitzScenes.stat.HasServantClothes) { return 1; } else { return 3; }
};
KrawitzScenes.EntitySuspicion = (entity: EncType) => {
	if (entity === EncType.Guard) {
...
KrawitzLoc.Grounds.enc.AddEnc(() => KrawitzScenes.PatrollingGuards, 1.0, () => !KrawitzScenes.stat.Orgy);
KrawitzLoc.Grounds.enc.AddEnc(() => KrawitzScenes.WanderingServants, 1.0, () => !KrawitzScenes.stat.ServantSpikedWine);
...
KrawitzLoc.Grounds.onEntry = () => {
	...
	enc.AddEnc(KrawitzScenes.PatrollingGuards,  1.0, () => !KrawitzScenes.stat.Orgy);
	enc.AddEnc(KrawitzScenes.WanderingServants, 1.0, () => !KrawitzScenes.stat.ServantSpikedWine);
...
KrawitzLoc.Mansion.Hall.links.push(new Link(
	"Study", true, () => KrawitzScenes.stat.Orgy || (!KrawitzScenes.stat.TFdKrawitz && KrawitzScenes.stat.KrawitzFood !== 3),
	() => {
...
KrawitzScenes.Flee = (entryPoint: boolean) => {
	const parse: any = {};
	if (!entryPoint) {
...
KrawitzScenes.GuardLost = (gender: Gender) => {
	const parse: any = {
...
KrawitzScenes.GuardConvinced = (gender: Gender) => {
	const parse: any = {
		HeShe  : gender === Gender.male ? "He" : "She",
		heshe  : gender === Gender.male ? "he" : "she",
		hisher : gender === Gender.male ? "his" : "her",
...
KrawitzScenes.ServantLost = (gender: Gender) => {
	const parse: any = {
		HeShe   : gender === Gender.male ? "He" : "She",
		heshe   : gender === Gender.male ? "he" : "she",
		hisher  : gender === Gender.male ? "his" : "her",
...
KrawitzScenes.ServantConvinced = (gender: Gender) => {
...
KrawitzScenes.FoundOut = (entity: EncType, num: number, gender?: Gender) => {
	const player = GAME().player;
...
KrawitzScenes.AddSuspicion = (num: number, surpressNext: boolean) => {
...
KrawitzScenes.PatrollingGuards = () => {
	const rigard = GAME().rigard;
	...
	let sceneId = KrawitzScenes.stat.guardRot;
...
KrawitzScenes.WanderingServants = () => {
	const parse: any = {
	...
	let sceneId = KrawitzScenes.stat.servantRot;
	if (sceneId >= scenes.length) { sceneId = 0; }
