...
export namespace MagicShopScenes {
	...
	export function CreateShop() {
		const buySuccessFunc = (item: Item, cost: number, num: number) => {
			...
			scenes.AddEnc(() => { Text.Add("<i>“Wonderful,”</i> the jackaless says as she takes your coins, flashing you a smile in return. <i>“Remember - not to be misusing things that Asche sells customer, for her guarantee of her goods not being harmful only holds if customer is not planning to do evil with it…”</i>", parse); });
			scenes.AddEnc(() => { Text.Add("Asche’s fingers flick the beads of her abacus as she sums up your total with lightning speed. <i>“Ah… there. Well, please to be taking ownership of new possession; may it be serving customer well. If you are having doubts, please to be taking it back to Asche, who will be explaining in more detail its use.”</i>", parse); });
			scenes.AddEnc(() => { Text.Add("With an almost lazy ease, Asche works her abacus with one hand while she counts your money with the other. Satisfied, she sweeps your coins to her side of the counter, then nudges your purchase to you. <i>“Please to be enjoying purchase. Asche is happy to help you out with your needs.”</i>", parse); });
			scenes.Get();
		...
		const buyPromptFunc = (item: Item, cost: number, bought: boolean) => {
			...
			const scenes = new EncounterTable();
			scenes.AddEnc(() => { Text.Add("<i>“Ah, so that is being customer’s desire? Asche can be selling it for [coin] coins. Price is always final, unless this jackaless is saying otherwise; she is knowing the worth of her stock.”</i>", parse); });
			scenes.AddEnc(() => { Text.Add("Asche peers at the [item] you’ve picked out. <i>“Oh, that one is being sold for [coin] coins. This jackaless is thinking that customer is making very good choice, may not be helpful in all situations, but then again, what is being that way? Is customer wishing to be buying?”</i>", parse); });
			scenes.AddEnc(() => { Text.Add("<i>“Customer’s choice is not exactly being what Asche expected, but is still being very fine nevertheless. Cost of that will be [coin] coins, [heshe] will be making payment, yes?”</i>", parse); });
			scenes.Get();
		...
		shop = new Shop({
			...
			sellFailFunc(item: Item, cost: number, sold: boolean) {
				...
				if (sold) {
					...
					const scenes = new EncounterTable();
					scenes.AddEnc(() => { Text.Add("Asche smiles and returns the item to you. <i>“Customer is not liking price Asche has given? Is quite natural, everyone is thinking their little personal treasure is worth more than it is. Alas, Asche cannot pay for sentimental value - is pity, but what can she do? Perhaps there is something else customer might be wanting?”</i>", parse); });
					scenes.AddEnc(() => { Text.Add("The jackal-morph smiles and casually pushes your offering back to you. <i>“Customer is to be suiting [himher]self; Asche does not force anyone to accept her prices. Maybe customer is coming back when [heshe] is changing [hisher] mind, yes?”</i>", parse); });
					scenes.Get();
				}
