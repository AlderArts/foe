...
ArmorShopLoc.description = () => {
	...
	scenes.AddEnc(() => { parse.sign = "PROTECT YOUR VULNERABLES"; });
	scenes.AddEnc(() => { parse.sign = "NO EXCHANGES"; });
	scenes.AddEnc(() => { parse.sign = "GUARANTEED SATISFACTION"; });
	scenes.AddEnc(() => { parse.sign = "NO HAGGLING"; });
	scenes.AddEnc(() => { parse.sign = "CAVEAT EMPTOR"; });
	scenes.AddEnc(() => { parse.sign = "YOU BREAK IT, YOU BUY IT"; });
	scenes.AddEnc(() => { parse.sign = "WATCH YOUR BELONGINGS"; });
	scenes.AddEnc(() => { parse.sign = "NO LOITERING"; });
	scenes.AddEnc(() => { parse.sign = "NO STRANGE ACTIVITIES WITH THE DISPLAYS"; });
	scenes.AddEnc(() => { parse.sign = "NO TAKEBACKS"; });
	scenes.AddEnc(() => { parse.sign = "WE ARE NOT RESPONSIBLE FOR INJURY OR LOSS"; });
	scenes.Get();
...
export namespace ArmorShopScenes {
	...
	export function CreateShop() {
		const buySuccessFunc = (item: Item, cost: number, num: number) => {
			...
			Text.Clear();
			const scenes = new EncounterTable();
			scenes.AddEnc(() => { Text.Add("<i>“Wonderful,”</i> Donovan says, making your money disappear somewhere on his person - you can’t tell where, he moves so quickly. <i>“I’d advise you to check your brand-new acquisition before leaving the premises. Once you’re out the doors, I’m not going to entertain any requests for refunds.”</i>", parse); });
			scenes.AddEnc(() => { Text.Add("Donovan’s smile doesn’t shift in the slightest as he counts and pockets your money with astounding speed. <i>“And thank you for shopping with us today, [sirmadam]. I’m sure your new purchase will serve you better than it did its previous owner.”</i>", parse); });
			scenes.AddEnc(() => { Text.Add("<i>“A very fine choice.”</i> Donovan pockets your money, then pulls out a small notebook and stub of pencil to record the sale. <i>“I’m sure that you’ll want something new, something stronger, something flashier in good time, so drop by every now and then. We rotate our stock on a regular basis.”</i>", parse); });
			scenes.AddEnc(() => { Text.Add("Donovan beams as he makes your money rapidly vanish into thin air - or at least, that’s what it looks like. <i>“An excellent decision. With all these strange goings-on beyond the city walls, I’m sure that a little extra protection can’t hurt when it comes to your vitals. Keep yourself safe, [sirmadam], and come back soon!”</i>", parse); });
			scenes.Get();
		...
		const buyFailFunc = (item: Item, cost: number, bought: boolean) => {
			...
			if (bought) {
				...
				const scenes = new EncounterTable();
				scenes.AddEnc(() => { Text.Add("A flicker of disappointment flashes across Donovan’s face, but he recovers with all the ease of an experienced salesman, quickly returning to his pleasant demeanor. <i>“Well, such a pity! Is there anything else I can do for you, [sirmadam]?”</i>", parse); });
				scenes.AddEnc(() => { Text.Add("<i>“Oh? Changed your mind? Well, I guess it wouldn’t have looked that good on you anyway.”</i> Donovan rubs his paws. <i>“Tell me, [sirmadam], would you be interested in looking at something else?”</i>", parse); });
				scenes.AddEnc(() => { Text.Add("Donovan winces, but clears his throat and pretends nothing happened. <i>“Oh. Well, you don’t want to do impulse buys, anyway - I want to my customers to leave truly happy with their purchases. Think I could interest you in something else?”</i>", parse); });
				scenes.AddEnc(() => { Text.Add("<i>“That one not to your liking? Was it the shine? The size? Or maybe it was a bad fit?”</i> Without waiting for your reply, Donovan begins to let both his gaze and paws wander about the aisles once more. <i>“Well, [sirmadam], don’t worry if that one didn’t quite tickle your fancy. I’ve got some more surprises waiting in the back…”</i>", parse); });
				scenes.Get();
			}
			Text.NL();
		};
		...
		const shop = new Shop({
			buyPromptFunc(item: Item, cost: number, bought: boolean) {
				...
				const scenes = new EncounterTable();
				scenes.AddEnc(() => { Text.Add("Donovan peers at your selection. <i>“Oh, [item]?” That’s a very lovely choice. Don’t mind the spot there, the previous owner wasn’t that good at dodging swords. How’s [coin] coins sound to you? A very fair deal, if I dare say so.”</i>", parse); });
				scenes.AddEnc(() => { Text.Add("<i>“That’s a very fine [item] you’ve picked out there, [sirmadam]. Used to belong to a little old lady, hardly ever used, she only put it on during weekends to watch her son in the city watch parades.”</i> Donovan rubs his paws together. <i>“[Coin] coins, and it’s yours.”</i>", parse); });
				scenes.AddEnc(() => { Text.Add("<i>“You’re thinking of making this [item] your very own, [sirmadam]? Can’t recommend this one enough - last owner had a sudden identity crisis and went off to be a cat or bat or rat or something. Whatever they ended up as, it didn’t fit him any more. Or her. Or it. You get the point.  Anyway, it’ll be [coin] coins if you want it.”</i>", parse); });
				scenes.AddEnc(() => { Text.Add("<i>“Oh, you have a good eye there. This [item] here’s City Watch surplus, seems like they didn’t have that many recruits in that size last year. You want to make it yours? [Coin] coins. Trust me, you won’t regret it.”</i>", parse); });
				scenes.AddEnc(() => { Text.Add("<i>“Excellent specimen of [item] you’ve got there, vintage - no, antique, even. I’d rather not call it battered; it’s got <b>character</b>, and that’s the important thing. Stood the test of time, it has, and it can be yours for only [coin] coins.”</i>", parse); });
				scenes.Get();
			...
			sellFailFunc(item: Item, cost: number, sold: boolean) {
				...
				if (sold) {
					...
					const scenes = new EncounterTable();
					scenes.AddEnc(() => { Text.Add("Donovan shrugs. <i>“Sorry we couldn’t do business, but I don’t haggle. My prices are what they are - already cutting my own throat on most of my deals as it is. Sometimes, a man gets tired of eating watery gruel and cabbage for dinner every night, you know? Well, anything else I can do for you?”</i>", parse); });
					scenes.AddEnc(() => { Text.Add("<i>“Changed your mind? I’m afraid I can’t go any higher on that; I’m already one to two months’ operating expenses from selling sausages in buns at the plaza. Still… is there anything I can do for you, since you’re here?”</i>", parse); });
					scenes.AddEnc(() => { Text.Add("<i>“Sorry, [sirmadam]. I can’t go any higher on that. Can’t make exceptions, won’t play favorites, gotta treat all customers with respect. You ever change your mind about changing your mind, and I’ll be more than happy to take it off your hands.”</i>", parse); });
					scenes.AddEnc(() => { Text.Add("Donovan lets out a small sigh as he pushes your [item] back to you and straightens his tie. <i>“Pity we couldn’t see eye-to-eye, [sirmadam]. Still, I’ll be here if you ever change your mind, so there’s no rush. Can I help you with something else?”</i>", parse); });
					scenes.Get();
				}
		...
		const specialShop = new Shop({
			buyPromptFunc(item: Item, cost: number) {
				...
				const scenes = new EncounterTable();
				scenes.AddEnc(() => { Text.Add("<i>“Oh-ho. I see you’ve a fine eye! That just happens to be a very rare and valuable [item] from another plane. Guaranteed to be free of any untoward ancient curses and the like - that’s the risk you run when you buy mysterious antiques from piddling little stores in street corners, but not my fine establishment. As for the price… how about [coin] coins? I’m cutting my own throat on this deal, I am!”</i>", parse); });
				scenes.AddEnc(() => { Text.Add("<i>“That? It’s a very fine [item] indeed. You don’t want to know what I had to go through just to get it. Really. Honestly. Don’t ask, okay? Not asking? Being very quiet? Very good. That’ll be [coin] coins if you want it.”</i>", parse); });
				scenes.AddEnc(() => { Text.Add("<i>“Now, what you have in your hands is a particularly exquisite specimen of [item]. There aren’t that many around these days - it’s said that none have been made since Riordane’s time. Of course, those are just rumors - or are they? In any case, it’ll be [coin] coins for it.”</i>", parse); });
				scenes.AddEnc(() => {
		...
		const prompt = () => {
			...
			options.push({ nameStr : "Buy",
				func() {
					Text.Clear();
					const scenes = new EncounterTable();
					scenes.AddEnc(() => { Text.Add("Donovan’s eyes light up, the stoat suddenly alert and excitable. <i>“A purchase? Marvellous! What can I interest you in today, then?”</i>", parse); });
					scenes.AddEnc(() => { Text.Add("Upon hearing your intentions, Donovan’s smile splits into a grin worthy of a shark. <i>“What excellent timing. There’re some lovely pieces I’d like to show you. Adventuring’s a dangerous job, you know, gotta protect your joints and all… might get an arrow in one of them, and that’d be the end of your stint in the business.”</i>", parse); });
					scenes.AddEnc(() => { Text.Add("<i>“Oh, the good [sirmadam] wants to buy something, does [heshe]?”</i> Donovan says with a chuckle. <i>“Well then, you’ve come to just the right place. Let me show you something of what we’ve got today…”</i>", parse); });
					scenes.AddEnc(() => { Text.Add("<i>“Oh, glorious, a customer!”</i> You have to admit, with an approach like that, the stoat-morph is doing a very nice job of making you feel like you’re wanted here… or at least, faking the appropriate attitude. <i>“And what can I get you today?”</i>", parse); });
					scenes.Get();

					rigard.ArmorShop.Buy(() => {
						Text.Clear();
			...
			options.push({ nameStr : "Sell",
				func() {
					Text.Clear();
					const scenes = new EncounterTable();
					scenes.AddEnc(() => { Text.Add("<i>“Ah! You have something you’d like me to take off your hands? Something too old, a little out of fashion, or maybe you’re outgrowing your first breastplate? Oh, no need to be shy, it happens.”</i> He rubs his paws together.", parse); });
					scenes.AddEnc(() => { Text.Add("<i>“Oh, in need of a bit of spare change? Well, I’ll be pleased to buy your old bits and pieces for a very fair and reasonable price.”</i> He gives you a nod and grin. <i>“Just let me take a look at it, and I’ll give you a quote.”</i>", parse); });
					scenes.AddEnc(() => { Text.Add("<i>“You’d like me to have a look at something? Well, we here at Twopenny’s only accept the best. Make sure your goods are in serviceable condition.”</i> Donovan coughs very deliberately.", parse); });
					scenes.AddEnc(() => { Text.Add("<i>“Looking to sell something? Remember the rules, then; always remember the rules. Don’t want to have to report my favorite customer to the watch, that would be a terrible, horrible thing for me to have to do, but I’d have to do it anyway.”</i>", parse); });
					scenes.Get();
					Text.NL();
