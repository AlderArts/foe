...
import { GAME, NAV, TimeStep, WORLD } from "../GAME";
...
const TerryScenes: any = {};

TerryScenes.JeanneTFCost = () => {
    return 50;
};

TerryScenes.Appearance = () => {
	...
	const parse: any = {
		...
		tbreastDesc() { return terry.FirstBreastRow().Short(); },
		tcockDesc() { return terry.MultiCockDesc(); },
	};
...
TerryScenes.ExploreResidential = () => {
	const terry = GAME().terry;
...
TerryScenes.CombatVsMiranda = () => {
 ...
 const world = WORLD();
	...
	enc.onLoss = () => {
		...
		}, 1.0, () => terry.hidingSpot !== world.loc.Rigard.Gate);
		scenes.AddEnc(() => {
			terry.hidingSpot = world.loc.Rigard.Residential.Street;
		}, 1.0, () => terry.hidingSpot !== world.loc.Rigard.Residential.Street);
		scenes.AddEnc(() => {
			terry.hidingSpot = world.loc.Rigard.ShopStreet.Street;
		}, 1.0, () => terry.hidingSpot !== world.loc.Rigard.ShopStreet.Street);
		scenes.AddEnc(() => {
			terry.hidingSpot = world.loc.Rigard.Plaza;
		}, 1.0, () => terry.hidingSpot !== world.loc.Rigard.Plaza);
...
TerryScenes.Release = () => {
 ...
 const world = WORLD();
	...
	const parse: any = {
		playername : player.name,
		masterMistress : player.mfTrue("master", "mistress"),
		armorTopDesc() { return player.ArmorDesc(); },
	};
...
TerryScenes.TalkFeelings = () => {
	...
	let parse: any = {
		playername: player.name,
		foxvixen : terry.mfPronoun("fox", "vixen"),
		breasts() { return terry.FirstBreastRow().Short(); },
...
TerryScenes.TalkPronoun = () => {
	const terry = GAME().terry;
	const player = GAME().player;

	const parse: any = {
		breasts() {
			const desc = terry.FirstBreastRow().Desc();
			return desc.cup + "s";
		},
		foxvixen : terry.mfPronoun("fox", "vixen"),
		cock() { return terry.MultiCockDesc(); },
		HeShe() { return terry.HeShe(); },
		heshe() { return terry.heshe(); },
		HisHer() { return terry.HisHer(); },
		hisher() { return terry.hisher(); },
		himher() { return terry.himher(); },
		hishers() { return terry.hishers(); },
		girlguy() { return terry.mfPronoun("guy", "girl"); },
	};
...
TerryScenes.TalkPast = (force: boolean) => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.SkinshipPromptChoices = () => {
	...
	options.push({ nameStr : "Hug",
		func() {
			...
			const pbreasts = player.FirstBreastRow().Size() > 2;
			const tbreasts = terry.FirstBreastRow().Size() > 2;
			Text.Clear();
...
TerryScenes.BrushTail = () => {
	...
	let parse: any = {
		...
		tarmorDesc() { return terry.ArmorDesc(); },
		tlowerArmor() { return terry.LowerArmorDesc(); },
		tbreastDesc() { return terry.FirstBreastRow().Short(); },
		tcockDesc() { return terry.MultiCockDesc(); },
		bellydesc() { return terry.StomachDesc(); },
...
TerryScenes.CheckFluids = () => {
	...
	let parse: any = {
		foxvixen       : terry.mfPronoun("fox", "vixen"),
		boygirl        : terry.mfPronoun("boy", "girl"),
		armorDesc() { return terry.ArmorDesc(); },
		topArmorDesc() { return terry.ArmorDesc(); },
		lowerArmorDesc() { return terry.LowerArmorDesc(); },
		tbreastDesc() { return terry.FirstBreastRow().Short(); },
		tcockDesc() { return terry.MultiCockDesc(); },
		bellydesc() { return terry.StomachDesc(); },
		playername : player.name,
...
TerryScenes.RosalinTF = () => {
	...
	let parse: any = {
		playername : player.name,
		rearsDesc() { return rosalin.EarDesc(true); },
		foxvixen   : terry.mfPronoun("fox", "vixen"),
	};
	parse = terry.ParserPronouns(parse);
...
TerryScenes.JeanneTFPrompt = () => {
	...
	const AddItem = (item: TFItem, scene: any, name: string, tooltip: string, costmult: number, horseTF?: boolean) => {
		options.push({ nameStr : name || item.name,
			func(obj: any) {
				parse.item = obj.str;
				const coin = obj.mult * TerryScenes.JeanneTFCost();
...
TerryScenes.JeanneTFCraft = (item: TFItem, scene: any, horseTF: boolean) => {
	const terry = GAME().terry;
	const player = GAME().player;
	...
	let parse: any = {
		...
		armorDesc() { return terry.ArmorDesc(); },
		terrycock() { return terry.MultiCockDesc(); },
		terrypussy() { return terry.FirstVag().Short(); },
	};
	parse = terry.ParserPronouns(parse);
	...
	Gui.NextPrompt(() => {
		...
		if (horseTF) {
			...
			parse.multiCockDesc = () => player.MultiCockDesc();
			parse.cockDesc      = () => p1Cock.Short();
			...
			TerryScenes.SexFuckButtEntrypoint(p1Cock, false, (rough: boolean) => {
				...
				Text.NL();
				parse.armorDesc = () => player.ArmorDesc();
...
TerryScenes.JeanneTFShrinkBoobs = () => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.JeanneTFStartLactate = () => {
	const terry = GAME().terry;
	const player = GAME().player;
	...
	let parse: any = {
		playername : player.name,
		foxvixen   : terry.mfPronoun("fox", "vixen"),
		terrybreasts() { return terry.FirstBreastRow().Short(); },
	};
	parse = terry.ParserPronouns(parse);
...
TerryScenes.JeanneTFStopLactate = () => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.JeanneTFGrowVag = () => {
	...
	const cum = terry.OrgasmCum();

	JeanneScenes.InteractPrompt();
};

TerryScenes.JeanneTFRemVag = () => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.JeanneTFGrowHorsecock = () => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.Impregnate = (mother: Entity, father: Entity, cum: number, slot?: number) => {
...
// TODO
TerryScenes.SexPrompt = (backPrompt: CallableFunction) => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.SexPromptChoice = (backPrompt: CallableFunction, haveadrink: boolean) => {
	const terry = GAME().terry;
	const player = GAME().player;
	...
	if (terry.FirstCock()) {
		if (player.FirstVag()) {
			const tooltip = terry.HorseCock() ? "With that great big monster between [hisher] legs, you just know [heshe] can give you a ride to remember." : "It’s not size that matters, it’s skill. Let’s see just how well your pet [foxvixen] can use that cock of [hishers].";
		...
		const tooltip = terry.HorseCock() ? "You went through the trouble of giving Terry that big stallionhood of [hishers] for a reason." : "Why not let the [foxvixen] catch a break and let [himher] do you for a change of pace?";
	...
	options.push({ nameStr : "Get oral",
		func() {
			...
			if (player.FirstVag()) {
				...
				const tooltip = terry.Relation() < 60 ? "Terry can start by licking your [vag]." : "Well, how about a nice dose of honey straight from your own pretty little flower?";
...
TerryScenes.SexGetOralPussy = () => {
	...
	const relslut = terry.Relation() + terry.Slut();
...
TerryScenes.SexGetOralCock = () => {
	...
	if (player.NumCocks() > 1) {
		...
		_.each(player.AllCocks(), (c, key: number) => {
			cockFunc(c, key);
		});
...
TerryScenes.SexGetOralCockCont = (parse: any, p1cock: Cock) => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.SexPitchAnal = (cocksInAss: Cock[]) => {
	...
	Gui.Callstack.push(() => {
		...
		Gui.Callstack.push(() => {
			...
			TerryScenes.SexFuckButtEntrypoint(p1Cock, promise, (rough: boolean) => {
				if (rough) {
...
TerryScenes.SexFuckButtEntrypoint = (p1Cock: Cock, promise: boolean, retFunc: any) => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.PCCleansTerry = (func: any, opts: any = {}) => {
	const terry = GAME().terry;
	const player = GAME().player;
	...
	options.push({ nameStr : "Towel",
		func() {
			...
			Text.NL();
			const preg = terry.PregHandler().IsPregnant();
	...
	const tooltip = player.Slut() < 30 ? "It's icky and gross, but you probably could lick [himher] clean... you think [heshe]'d like that... Honestly, it's kind of hot, too." :
...
TerryScenes.TCleansPC = (func: any, opts: any = {}) => {
	const terry = GAME().terry;
	const player = GAME().player;
	const party: Party = GAME().party;
...
TerryScenes.FuckedByBunnyMob = (male: Entity, parse: any) => {
	const terry = GAME().terry;
	...
	let pussy = false;
	if (terry.FirstVag()) {
		const virgin = terry.FirstVag().virgin;
	...
	Text.NL();
	if (!pussy) {
		const avirgin = terry.Butt().virgin;
...
TerryScenes.SexPitchVaginal = (cocks: Cock[]) => {
	const terry = GAME().terry;
	const player = GAME().player;
	const party: Party = GAME().party;
	...
	const virgin = terry.FirstVag().virgin;
	const virginFirst = terry.PussyVirgin();
	...
	const dildoScene = (toy: any) => {
		...
		const belly = terry.PregHandler().BellySize();
		if (belly < 0.2) {
			...
			if (terry.Cup() >= TerryFlags.Breasts.Ccup) {
				options.push({ nameStr : "Tittyjob",
					func() {
						...
						const tail = player.IsNaga() || player.HasPrehensileTail();
						if (tail) {
							parse.ti = player.IsNaga() ? () => player.LegsDesc() : player.HasTail().Short();
...
TerryScenes.SexCatchVaginal = () => {
	...
	if (terry.Relation() < 30) {
		...
		const nFucked = terry.sex.rAnal + terry.sex.rVag;
		if (nFucked >= 5) {
	...
	if (terry.HorseCock()) {
		...
		options.push({nameStr : Text.Parse("[HeShe] decides", parse),
			...
			enabled : true,
			func() {
				const inside = TerryScenes.SexCatchVaginalHeChooses();
...
TerryScenes.SexCatchVaginalHeChooses = () => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.SexCatchVaginalInsideHorseCock = (parse: any) => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.SexCatchVaginalOutsideHorseCock = (parse: any) => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.SexCatchVaginalInsideFoxCock = (parse: any) => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.SexCatchVaginalOutsideFoxCock = (parse: any) => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.SexCatchAnal = () => {
	...
	const first = terry.flags.caFirst === 0;
	terry.flags.caFirst++;
...
TerryScenes.SexHaveADrink = (back: any) => {
	const terry = GAME().terry;
...
TerryScenes.SexHaveADrinkPussy = () => {
	const terry = GAME().terry;
	const player = GAME().player;
	...
	const rel = terry.Relation() + terry.Slut();
	...
	const womb = terry.PregHandler().Womb();
	const preg = womb && womb.pregnant;
	const stage = womb && womb.progress;
...
TerryScenes.SexHaveADrinkBreasts = () => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.SexHaveADrinkBreastsMilk = (parse: any) => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.SexHaveADrinkBreastsRomance = (parse: any) => {
	const terry = GAME().terry;
...
TerryScenes.SexHaveADrinkBreastsArousal = (parse: any) => {
	const terry = GAME().terry;
	// #arousal check block
...
TerryScenes.SexHaveADrinkBreastsArousalHorsecock = (parse: any) => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.SexHaveADrinkBreastsArousalFoxcock = (parse: any) => {
	const terry = GAME().terry;
	const player = GAME().player;
...
TerryScenes.SexHaveADrinkBreastsArousalPussy = (parse: any) => {
	const terry = GAME().terry;
	const player = GAME().player;
