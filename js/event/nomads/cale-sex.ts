...
/* CALE SEX SCENES */
const CaleSexScenes: any = {};

CaleSexScenes.Impregnate = (mother: Entity, slot: number, cum: number) => {
...
CaleSexScenes.TentSex = () => {
	...
	Gui.SetButtonsFromList(options, true, () => {
		...
		CaleSexScenes.Prompt();
	});
};

CaleSexScenes.OutsideSex = () => {
	const player = GAME().player;
...
CaleSexScenes.SexSuckHim = (outside: boolean) => {
	const player = GAME().player;
	const parse: any = {
...
CaleSexScenes.SexSuckHimEntryPoint = (outside: boolean) => {
	const player = GAME().player;
	const cale = GAME().cale;
...
CaleSexScenes.SexGetTrickBJ = (outside: boolean) => {
	const player = GAME().player;
	const cale = GAME().cale;
	const virgin = cale.Butt().virgin;
...
CaleSexScenes.SexGetBJ = (outside: boolean) => {
	const player = GAME().player;
	const cale = GAME().cale;
	const parse: any = {
...
CaleSexScenes.SexGetBJSneakyEntry = (outside: boolean, sneaky: boolean) => {
	const player = GAME().player;
	const cale = GAME().cale;
...
CaleSexScenes.SexGetEatenEntrypoint = (outside: boolean) => {
	const player = GAME().player;
	const cale = GAME().cale;
	const virgin = cale.Butt().virgin;
...
CaleSexScenes.SexFuckHim = (outside: boolean, opts?: any) => {
	const player = GAME().player;
	const cale = GAME().cale;
	opts = opts || {};
...
CaleSexScenes.SexCatchVag = (outside: boolean) => {
	const player = GAME().player;
	const cale = GAME().cale;
	let parse: any = {
		log           : outside ? "log" : "bedroll",
...
CaleSexScenes.SexCaleShowerEntrypoint = (outside: boolean) => {
	const player = GAME().player;
	const cale = GAME().cale;
	let parse: any = {
		log        : outside ? "log" : "bedroll",
...
CaleSexScenes.SexCaleCleanCockEntrypoint = (cock: Cock, outside: boolean) => {
	const player = GAME().player;
	let parse: any = {
...
CaleSexScenes.SexCaleButtslutEntrypoint = (cocks: Cock[], outside: boolean) => {
	const player = GAME().player;
	const cale = GAME().cale;
...
CaleSexScenes.SexCatchAnal = (outside: boolean) => {
	const player = GAME().player;
	const cale = GAME().cale;
	let parse: any = {
		log : outside ? "log" : "bedroll",
...
CaleSexScenes.SexCatchAnalEntrypoint = (outside: any, fromVag: boolean) => {
	const player = GAME().player;
	const cale = GAME().cale;
	const cocksInAss = player.CocksThatFit(cale.Butt(), true);
...
CaleSexScenes.SexFuckingHimOutsideComments = (cock: Cock, opts: any) => {
	const player = GAME().player;
	const rosalin = GAME().rosalin;
	const cale = GAME().cale;
