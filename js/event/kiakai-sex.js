	...
	if(player.HPLevel() < 0.25) { // REALLY LOW HEALTH 0-25%
		...
		options.push({ nameStr : "Seduce",
			func : function() {
				...
				if(kiakai.flags["Sexed"] > 15) {
					...
					Text.Add("<i>“I... I think you are right, [playername]!”</i> [name] quickly wriggles out of [hisher] [armor], breathing a bit faster. [HeShe] only hesitates for a moment before greedily probing your naked body, gently letting [hisher] fingers trace [shape].", parse);
				}
				else {
				...
				if(kiakai.flags["Attitude"] > Kiakai.Attitude.Neutral)
					Text.Add("[HeShe] doesn't seem to mind at all, sighing happily and snuggling up against your [breastDesc].", parse);
		...
		options.push({ nameStr : "Insist",
			func : function() {
				...
				Text.NL();
				Text.Add("The elf's face turns a bright pink, then beet red, as you guide [hisher] fingers across your body. Their first stop is your [breastDesc], stopping to circle your [nipsDesc] before trailing down toward your exposed crotch.", parse);
				Text.NL();
				Text.Add("You hold [name]'s gaze captive, the poor elf almost hypnotized as [hisher] hand inexorably draws closer to your [genDesc]. You stop just short of it, forcing [himher] to close the final distance on [hisher] own.", parse);
				Text.NL();
				
				if(kiakai.flags["Sexed"] > 10) {
					Text.Add("Without hesitation, the elf's fingers ", parse);
					if(player.FirstCock())
						Text.Add("close around your erect [cockDesc], caressing the length reverently.",parse);
					else if(player.FirstVag())
						Text.Add("brush against your [vagDesc], gently teasing your folds apart.",parse);
					else
						Text.Add("caress your featureless crotch.");
					
					Text.Add(" <i>“You are always making me do weird things,”</i> [name] murmurs under [hisher] breath, but shrugs out of [hisher] [armor] without any further coaxing from you. Leaning back onto the ground, you enjoy the show.", parse);
				}
				else {
					...
					Text.NL();
					Text.Add("You console the flustered and protesting elf, telling [himher] that [heshe] is doing well; together, you have found the place that hurts, now [name] only needs to take care of it. Helping the brightly blushing elf out of [hisher] [armor], you recline and motion for [himher] to join you.", parse);
				}
				parse.att1 = kiakai.flags["Sexed"] > 20 ? "eagerly eyeing" : kiakai.flags["Sexed"] > 10 ? "pleasantly aware of" : "uncomfortably aware of";
				Text.NL();
				Text.Add("Once nude, the elf kneels down before you, [att1] your exposed [genDesc]. [HeShe] seems to be waiting for your instructions.", parse);
...
Scenes.Kiakai.HealingSeducePrompt = function() {
	...
	options.push({ nameStr : "Seduce",
		func : function() {
			...
			Text.Add("<i>“Should we not be going, [playername]?”</i> [name] mumbles, blushing slightly. You chuckle, insisting that you are in no rush. Lazily, you [l], giving the embarrassed elf a clear view of your [genDesc]. The elf gets down beside you, trying to avoid your gaze and cover [hisher] burning cheeks.", parse);
			Text.NL();
			Text.Add("A jolt runs up the elf's spine as you languidly caress [hisher] [hairDesc], whispering in [hisher] ear that [heshe] should get out of those pesky clothes.", parse);
			Text.NL();
			if(kiakai.flags["Sexed"] > 10)
				Text.Add("<i>“Every time we do this, I-”</i>, [name] cuts [himher]self off, blushing like a bride on her wedding day. Nonetheless, the elf snuggles down beside you, voluntarily removing [hisher] [armor].", parse);
			else
	...
	options.push({ nameStr : "Assert",
		func : function() {
			...
			if(kiakai.flags["Sexed"] > 10) {
				...
				Text.Add("<i>“Don't be coy, now,”</i> you say teasingly,[l] presenting the brightly blushing elf with your [genDesc]. <i>“By now, you should know <b>exactly</b> what I want.”</i> Nervously, [name] shrugs out of [hisher] [armor], [hisher] own arousal getting the best of [himher]. As [heshe] gets down on [hisher] knees[l2], [name] looks up at you, itching to get started but awaiting your word.", parse);
			}
			else {
				...
				Text.NL();
				Text.Add("A bit surprisingly, the elf is quick to catch on, hurriedly getting out of [hisher] [armor] before kneeling down, an almost expectant look in [hisher] eyes.", parse);
			}
	...
	if(player.FirstCock()) {
		...
		options.push({ nameStr : "Handjob",
			func : function() {
				...
				Text.NL();
				Text.Add("Your intertwined hands close around your [cockDesc], the elf's lithe, slightly cold digits making you shudder in pleasure. At first, you have to lead [himher] through the motion, but soon you withdraw your hand, giving the horny elf free rein on your [cockDesc].", parse);
				Text.NL();
				...
				if(kiakai.FirstCock()) {
					...
					if(Math.random() < 0.5) {
						Text.Add("[name]'s own [kMultiCockDesc] stand[notS] at attention, tightly trapped between you. Amused, you reach down and fondle [itThem] before letting the elf get back to business. ", parse);
						kiakai.AddLustFraction(0.3);
					}
					else {
						Text.Add("[name] suddenly moans as [hisher] own [kMultiCockDesc], which [hasHave] been rubbing against you for some time, erupt[notS] as the elf prematurely climaxes. You hold on to [himher] as [heshe] rides out the wave, whispering encouragements. ", parse);
				...
				if(kiakai.FirstBreastRow().size.Get() >= 3) {
					Text.Add("[HisHer] [kBreastDesc] mash against you, the perky nipples stiff with [hisher] lust.", parse);
				...
				var scenes = new EncounterTable();
				scenes.AddEnc(function() {
					Text.Add("A jolt in your nethers snaps you into full attention, and suddenly you are fighting to hold back your release. Glancing down, you see that [name] is not even touching you anymore - rather, a small spark of healing magic bridges the gap to your [cockDesc].", parse);
					Text.NL();
					...
					scenes.AddEnc(function() {
						Text.Add("Grinning, you guide [name]'s head down to your still erupting [cockDesc]. ", parse);
						if(kiakai.flags["Sexed"] > 15)
				...
				scenes.AddEnc(function() {
					Text.Add("Edged on by your encouragement, [name] shifts around so that [heshe] is kneeling beside your prone form, leaving [himher] with full access to your [cockDesc]. [HeShe] grasps the erect member with both hands, trying [hisher] best to pleasure you.", parse);
					Text.NL();
					Text.Add("<i>“Just... like... that!”</i> You moan, urging [himher] to keep up the pace. Understanding dawns on [name]'s face, blushing at your praise, [heshe] keeps jerking your [cockDesc], determined to make you cum. [HeShe] isn't forced to wait long.", parse);
					Text.NL();
					...
					scenes.AddEnc(function() {
						...
						if(player.CumOutput() > 3)
							Text.Add("your [cockDesc] exploding with cum in the poor elf's hands.", parse);
						else
							Text.Add("your seed pulsing from your [cockDesc] and trailing down the elf's hands.", parse);
						Text.NL();
					}, 1.0);
					scenes.AddEnc(function() {
						Text.Add("In a rather uncharacteristic move, [name] leans down to plant a kiss on your trembling [cockDesc]. This plan backfires though, as you pick that exact moment to hit your peak.", parse);
						Text.NL();
						...
						if(player.CumOutput() > 3) {
							...
							Text.NL();
							Text.Add("The elf noisily swallows wad after wad of sperm, hopelessly trying to keep up with your output. Finished, you relinquish your hold and allow [himher] to take [hisher] lips off your [cockDesc].", parse);
						}
						else if(kiakai.flags["Sexed"] > 15) {
		...
		options.push({ nameStr : "Blowjob",
			func : function() {
				...
				Text.NL();
				Text.Add("[name] begins by placing a track of glistering pecks on your [cockDesc], covering the entire length with kisses.", parse);
				Text.NL();
				
				if(player.NumCocks() > 1) {
					Text.Add("Deciding that it would be unfair to focus all [hisher] attention on only <i>one</i> of your members, [name] repeats the same process for each of your [multiCockDesc].", parse);
					Text.NL();
				}
				
				if(player.HasBalls()) {
					Text.Add("When reaching your scrotum, [heshe] gives each of your [ballsDesc] a long lick, sucking a bit on them before returning [hisher] attention to the main course.", parse);
					Text.NL();
				}
				else if(player.FirstVag()) {
					Text.Add("As [heshe] reaches the base of your [cockDesc], the elf is greeted by your other genitalia, your moist [vagDesc]. [HeShe] gives it a few licks before returning to [hisher] original target.", parse);
					Text.NL();
				}
				...
				options.push({ nameStr : "Passive",
					func : function() {
						...
						if(kiakai.flags["Sexed"] > 15) {
							Text.Add("More than willing to step up to the challenge, the previously prim and proper [priest] gives [hisher] best to provide you with a good time. Sucking and slurping on your [cockDesc] like a starving [manwoman] who just found food, [name] is soon ", parse);
							...
							Text.Add("Even if [heshe] is still slightly inexperienced, [name] gives [hisher] best effort to get you off. Really getting down to business, the elf starts to suck on your [cockDesc], trying to fit as much of your length as possible into [hisher] mouth. ", parse);
							if(player.FirstCock().Size() > 60)
								Text.Add("Unable to take all of you, [name] lets your [cockDesc] momentarily press against the back of [hisher] throat, before hurriedly backing off.", parse);
							else
								Text.Add("Somehow, [heshe] manages to down all of your [cockDesc] without choking, though [heshe] seems to be having some trouble.", parse);
						}
						else {
						...
						*/
						...
						scenes.AddEnc(function() {
							if(player.FirstVag() && Math.random() < 0.5) {
								Text.Add("You are suddenly distracted from the exquisite blowjob by two lithe fingers, probing at your moist [vagDesc]. Gasping in surprise, you moan appreciatively as the elf slowly begins to touch your female parts.", parse);
								Text.NL();
								if(kiakai.flags["Attitude"] < Kiakai.Attitude.Neutral) {
									Text.Add("<i>“Good, showing some initiative!”</i> You grin, accentuating your words with a sudden thrust of your hips, forcing more of your [cockDesc] into [name]'s mouth.", parse);
									Text.NL();
								}
							...
							// Anal variation
								...
								Text.Add("[name] gazes up at you through a veil of lust clouding [hisher] eyes. As if trying to find other ways to bring you pleasure, [heshe] briefly takes your [cockDesc] out of [hisher] mouth, allowing [himher] to lather [hisher] digits in the sticky blend of saliva and pre-cum coating your member. Quickly getting back to the task at hand, [name] lets [hisher] wet fingers trail across your [skin], ", parse);
								if(player.HasBalls())
									Text.Add("cupping and fondling your [ballsDesc], ", parse);
								Text.Add("before finding their target.", parse);
								Text.NL();
								Text.Add("You draw a surprised gasp as you feel two of the elf's lubed fingers prod at your [anusDesc]. [name] looks up uncertainly, as if asking permission to continue.", parse);
								Text.Flush();
								//[Allow][Deny][Punish]
								var options = new Array();
						...
						scenes.AddEnc(function() {
							if(kiakai.flags["Sexed"] > 10) {
								Text.Add("The sudden tightening of your [cockDesc] gives the elf all the warning needed. [HeShe] begins to deep-throat you repeatedly, massaging you with the muscles at the back of [hisher] throat. Moaning loudly, you give [name] what [heshe] wants: a generous serving of hot spunk, delivered directly from the tap. ", parse);
								var cum = player.OrgasmCum();
								
								if(cum > 3)
									Text.Add("No amount of preparation could prepare [himher] for the size of your load, and when the elf has had [hisher] fill, [heshe] lets your [cockDesc] flop free, hosing [himher]self in your semen.", parse);
								else
									Text.Add("When you are done, the elf cleans your [cockDesc], not wanting to let even a drop go to waste.", parse);
							}
							else {
				...
				options.push({ nameStr : "Active",
					func : function() {
						...
						player.AddSexExp(1);
						kiakai.AddSexExp(1);
						Text.Add("[name]'s [eyeColor] eyes snap open in surprise as you suddenly thrust much more of your [cockDesc] into [hisher] mouth than [heshe] was ready for.", parse);
						Text.NL();
						
						if(kiakai.flags["Sexed"] > 10) {
							Text.Add("Without missing a beat, [name] meets you thrust for thrust, [hisher] well-trained mouth wrapping around your [cockDesc]. ", parse);
							if(player.FirstCock().Size() > 75) {
								...
								Text.Add("Too big to fit in just [hisher] mouth, an audible popping noise accompanies your [cockDesc] entering [hisher] throat. You praise your companion on [hisher] skills as [hisher] lips connect with your crotch, the entirety of your [cockDesc] lodged deep inside [himher].", parse);
							}
							else {
		...
		options.push({ nameStr : "Eat you",
			func : function() {
				...
				if(player.FirstVag().clitCock) {
					...
					Text.Add("As [name] gently licks at your [clitDesc], [hisher] nose keeps bumping against your [multiCockDesc], making [itThem] impossible to ignore. [name] gives [eachIt] a thorough lathering before returning to the task at hand.", parse);
					if(player.HasBalls())
						Text.Add(" As [hisher] [kTongueDesc] returns to work on your feminine parts, [name] caresses your [ballsDesc], sending shivers up your spine.", parse);
				}
				else if(kiakai.flags["Sexed"] > 15) {
					Text.Add("[name] knows just what to do. Expertly, [heshe] licks and sucks at your [clitDesc], sending electrical tingles through your body. [HeShe] is getting quite good at this sort of thing!", parse);
				}
				else {
					Text.Add("What [heshe] lacks in skill, [heshe] makes up for in dedication. After licking your [clitDesc] for a bit, [name] returns [hisher] attention to your wet [vagDesc], [hisher] nose occasionally butting against your button.", parse);
				}
				...
				scenes.AddEnc(function() {
					...
					// Regular fingering
						...
						Text.Add("Growing bolder by the minute, the horny elf adds another finger to [hisher] excavation team, pressing each of them knuckle-deep inside your tunnel. Your trembling [vagDesc] is gushing with girly juices, and lewd wet sounds come out from you at each of [hisher] thrusts.", parse);
						Text.NL();
						
						var cum = player.OrgasmCum();
						
						Text.Add("Finally, with a shuddering moan, you cum. Your [legsDesc] squirm as you shake uncontrollably, gushing sweet nectar all around your lover's fingers.", parse);
						if(player.FirstCock()) {
							parse["s"] = player.NumCocks() > 1 ? "" : "s";
							Text.Add(" The fluids dripping from your [vagDesc] are not the only things released by your orgasm though, as your [multiCockDesc] cover[s] the elf with thick white strands.", parse);
						}
						Text.NL();
				...
				scenes.AddEnc(function() {
					...
					Text.Add("Without letting up [hisher] assault on your netherlips, [name] gently tugs on your [multiCockDesc], determined to pleasure your male endowment[s] as well. [HisHer] deft fingers caress your length[s], playfully tracing the veins on the pulsating member[s].", parse);
					Text.NL();
					...
					if(kiakai.flags["Sexed"] > 15 && Math.random() > 0.5) {
						...
						if(cum > 3) {
							...
							Text.Add("Although [heshe] manages to gulp down the entire load from one of your [multiCockDesc], [count] left unattended, splattering the elf's face, hair, and clothes in your warm cum.", parse);
							Text.NL();
						}
						parse["l"] = player.HasLegs() ? "Between your legs" : "In your loins";
						Text.Add("[l], your [vagDesc] has clamped down on [name]'s fingers, coating them in slick juices. When the torrent of your orgasm has quieted down, the elf methodically licks the liquid from [hisher] digits, before providing your [vagDesc] with the same service.", parse);
					}
					else if(kiakai.flags["Sexed"] > 15) {
						Text.Add("Burying [hisher] tongue in your [vagDesc], [hisher] nose bumping up against your [clitDesc], [name] insistently laps at your inner tunnel, coaxing an orgasm from your burning body.", parse);
						Text.NL();
						...
						Text.Add("Suddenly overwhelmed by all the options in front of [himher], [name] panics slightly. Noticing that your [multiCockDesc] [isAre] about to erupt, [heshe] ineffectually tries to hold back the flow with both hands. This only succeeds in pushing you over the edge, staining [hisher] hands in your semen.", parse);
						Text.NL();
						
						var cum = player.OrgasmCum();
						
						Text.Add("Slightly flustered, [name] tries to focus on cleaning your [vagDesc], trying to ignore the white strands dripping down your cock[s], landing on [hisher] nose.", parse);
					}
					Text.Flush();
					Gui.NextPrompt();
				}, 1.0, function() { return player.FirstCock(); });
				scenes.AddEnc(function() {
					Text.Add("By this time, [name] knows just what to do to get you off. Electric shocks run through your tingling labia as [heshe] administers a spark of healing energy with the tip of [hisher] [kTongueDesc]. The sparks quickly home in on the most prominent feature of your feminine anatomy, your [clitDesc].", parse);
					Text.NL();
					Text.Add("The elf isn't even touching you anymore; [heshe] is just letting [hisher] healing power do the work. [HeShe] lets [hisher] [kTongueDesc] hover a hair's breadth away from your [vagDesc], a tiny beam of refreshing energy bridging the gap between the two of you.", parse);
					Text.NL();
					...
					if(player.FirstCock()) {
						parse["s"] = player.NumCocks() == 1 ? "s" : "";
						Text.Add("Your [multiCockDesc] erupt[s], covering [name]'s hair and back with sticky fluids.", parse);
	...
	options.push({ nameStr : "Cuddle",
		func : function() {
			...
			if(player.FirstBreastRow().size.Get() > 3)
				Text.Add(", your [breastDesc] pressing against [hisher] back", parse);
			Text.Add(". Your hands, trailing down [name]'s body, brush lightly over [hisher] [kNipsDesc], eliciting a surprised exhalation from the elf, before settling on [hisher] stomach. You find the nearness of the elf comforting. The warmth of your bodies intermingle as you lie there, ", parse);
...
Scenes.Kiakai.PleasureElf = function() {
	...
	if(kiakai.FirstCock()) {
		Text.Add("Without you even touching [itThem], [name]'s [kMultiCockDesc] spring[notS] to attention, stiff in anticipation. You take your time teasing the elf, letting your hand trail up and down [hisher] thigh, circle around [hisher] crotch and proceed down the other leg.", parse);
		Text.NL();
		...
		if(kiakai.HasBalls())
			Text.Add(" You let your hand come around, lightly cupping the elf's [kBallsDesc].", parse);
		Text.NL();
		...
		options.push({ nameStr : "Handjob",
			func : function() {
				...
				Text.Add("You delicately grasp[oneof] [name]'s [kMultiCockDesc].", parse);
				if(kiakai.FirstCock().length.Get() >= 20)
					...
					Text.Add(" Your free hand continues to play with [hisher] [kBallsDesc], massaging each testicle between your fingers.", parse);
				else
					Text.Add(" Your free hand keeps on exploring [name]'s body, never staying in one place for long.", parse);
				Text.NL();
				Text.Add("The elf gasps as you slowly begin to jerk [hisher] [kCockDesc], massaging it from tip to stem.", parse);
				Text.NL();
				...
				Text.NL();
				Text.Add("Taking pity on the elf, you grab hold of [hisher] [kCockDesc] again, and this time you don't hold back. [name]'s eyes are closed, and [hisher] hips make small thrusting motions as you rapidly pump [hisher] member.", parse);
				Text.NL();
				Text.Add("<i>“C-cumming!”</i> the elf squeaks. True to [hisher] word, [name]'s [kMultiCockDesc] twitch[notEs] in your hand[s], spurting elven cockbatter onto [hisher] own stomach. You give [himher] a kiss on the lips before cleaning up. Looks like the elf will need a few minutes to recover before you can set out again.", parse);
				Text.NL();
		...
		options.push({ nameStr : "Blowjob",
			func : function() {
				Text.Clear();
				Text.Add("You intend to do far more than touch. To get [himher] started, you grab hold of[oneof] [name]'s [kMultiCockDesc] and give it a few strokes. ", parse);
				if(kiakai.flags["Sexed"] < 10)
				...
				Text.NL();
				Text.Add("[name] cries out as your lips close around the [kCockTip] of [hisher] cock, your [tongueDesc] lapping away at it avidly. Your taste buds are assaulted by salty drops of pre landing on your [tongueDesc], dispensed from the elf's stiff member.", parse);
				Text.NL();
				Text.Add("That seemed to have a nice effect. How about stepping it up a notch?", parse);
				Text.NL();
				Text.Add("Slowly, you inch [name]'s [kCockDesc] into your mouth, coating the moaning elf's length in your slick saliva. Letting your [tongueDesc] stroke the stem playfully, you start bobbing your head up and down, eagerly swallowing your companion's trembling dick.", parse);
				Text.NL();
				if(kiakai.flags["Sexed"] < 10)
				...
				if(kiakai.FirstCock().length.Get() > 35) {
					...
					parse["len"] = len > 50 ? ", though it's still small in comparison to your own" :
					               len > 30 ? ", rivalling your own in size" :
					               ", easily dwarfing your own";
					Text.Add("It's almost unbelievable how large [name]'s member has grown[len]. Much too big for you to fully please, sadly enough. Still, you take as much of the [kCockDesc] in as you can, eyes watering slightly as you force it down your burning throat.", parse);
					Text.NL();
					...
					Text.Add("With some difficulty, you manage to push all of [name]'s [kCockDesc] down your throat, managing to press a trembling kiss on the elf's crotch before you[sl] surface for air.", parse);
					Text.NL();
				...
				Text.NL();
				Text.Add("<i>“B-by Aria, [playername], that feels amazing!”</i> the elf moans. You surface for a moment, a strand of saliva still connecting your [tongueDesc] with [hisher] [kCockDesc]. Flushed and panting, you manage to express that your diligent healer deserves no less, before returning to the task at hand.", parse);
				Text.NL();
				parse["mouthThroat"] = (kiakai.FirstCock().length.Get() > 15) ? "throat" : "mouth";
				Text.Add("[name] is in heaven, resting on [hisher] back, [hisher] [kCockDesc] buried deep in your [mouthThroat]. You catch fragments of what sounds like a jumbled prayer, delivered by the elf in a breathless whisper.", parse);
				Text.NL();
				if(kiakai.FirstVag()) {
					Text.Add("Not wanting to let [hisher] other genitalia go unrewarded, you push two fingers into [name]'s [kVagDesc], immediately soaking the digits in the elf's juices.", parse);
					Text.NL();
				}
				else if(kiakai.HasBalls()) {
					Text.Add("You cup [name]'s [kBallsDesc] in your hand, fondling and squeezing each testicle gently, trying to coax their load from them.", parse);
					Text.NL();
				}
				...
				if(kiakai.strength.Get() > player.strength.Get()) {
					...
					if(cum > 3)
						Text.Add(" You begin to get slightly worried as you can feel your stomach bulging slightly under the sheer amount of semen being pumped down your throat. Just how much cum can [name] produce?!", parse);
					Text.NL();
	...
	if(kiakai.FirstVag()) {
		...
		Text.Add("[name] is getting worked up, [hisher] [kVagDesc] already moist with juices. Teasing the elf, you caress the sensitive skin between [hisher] legs, trailing closer but never touching. [HeShe] moans in frustration.", parse);
		Text.NL();
		...
		options.push({ nameStr : "Eat out",
			func : function() {
				...
				Text.NL();
				Text.Add("[name] whimpers quietly as you place a peck on [hisher] exposed netherlips; the first of many to come. [name]'s sweet taste teases your taste buds, as if imploring you to dig in, to allow your [tongueDesc] to ravage and penetrate the innocent elf to [hisher] very core. The elf in question doesn't seem to mind, [hisher] whimper turning to a moan as your second kiss lingers on [hisher] [kVagDesc], your [tongueDesc] teasing the entrance.", parse);
				Text.NL();
				Text.Add("Leaning back, you admire [name]'s exposed pink flower, gently spread by your fingers. The opening of [hisher] passage is glistening wetly, eagerly awaiting your probing tongue. You give [himher] another long lick, your nose bumping up against the elf's sensitive [kClitDesc].", parse);
				Text.NL();
				Text.Add("<i>“Mm... [playername]...”</i> [name] sighs contentedly, one hand idly caressing [hisher] [kBreastDesc].", parse);
				Text.NL();
				...
				{
					Text.Add("Lapping away at the buffet laid out before you, you allow your [tongueDesc] to play across the lips of [hisher] labia, and taste the deeper reaches of [hisher] folds.", parse);
				}
				Text.NL();
	...
	options.push({ nameStr : "Ass",
		func : function() {
			...
			if(player.FirstCock())
				Text.Add(", blushing slightly as [heshe] feels your [multiCockDesc] push against [hisher] back", parse);
			...
			if(kiakai.flags["AnalExp"] < 5) {
				Text.Add("<i>“W-what are you doing, [playername]?”</i> [name] looks at you, bewildered. You smile mischievously, letting your lubricated digits prod at [hisher] [kAnusDesc]. What does [heshe] <i>think</i> you are doing?", parse);
				Text.NL();
				...
				Text.Add("<i>“P-please, [playername],”</i> [name] pleads, begging you to reconsider as you eye [hisher] [kAnusDesc], flexing your digits experimentally. No matter what the elf says, you notice that [heshe] doesn't resist you in the slightest, even going as far as to take a firmer grip of [hisher] calves, spreading [hisher] cheeks slightly wider.", parse);
			}
			else if(kiakai.flags["AnalExp"] < 20) {
				Text.Add("<i>“O-okay,”</i> [name] mumbles nervously, watching your preparations with conflicting emotions playing out on [hisher] face. When you experimentally prod at [hisher] [kAnusDesc], the elf blushes, but parts [hisher] legs slightly more, allowing you easier access.", parse);
			}
			else { // > 20
				Text.Add("<i>“M-make me feel good,”</i> [name] begs you, willingly spreading [hisher] legs farther apart to present you with [hisher] [kAnusDesc]. Not to disappoint, you rub your digits against [hisher] eager pucker, teasing and spreading it. The excited elf's breathing grows faster in anticipation, and [heshe] all but begs you to penetrate [himher].", parse);
			}
			Text.NL();
			Text.Add("[name] cries out as you push first one, then two fingers into [hisher] exposed [kAnusDesc]. You begin to pump your soaked digits in and out of [himher], adding a third finger once you have [himher] loosened enough.", parse);
			Text.NL();
			...
			if(kiakai.FirstCock()) {
				...
				if(kiakai.FirstCock().length.Get() > 25) {
					...
					Text.NL();
					if(kiakai.flags["Sexed"] < 20)
						Text.Add("Too flustered to even speak, [name] avidly shakes [hisher] head, denying that [heshe] would even consider something like that. Chuckling, you return your attention to thrusting your fingers into [hisher] ass, [name]'s [kMultiCockDesc] occasionally rubbing against the bewildered elf's lips or nose, leaving sticky trails of pre-cum wherever [itThey] make contact.", parse);
					else {
						Text.Add("As if [heshe] was only waiting for your permission, [name] starts to eagerly lick and suck at [hisher] own cock, rocking [hisher] hips slightly to give [himher]self a blowjob. Returning your attention to the elf's [kAnusDesc], you teasingly give the underside of [hisher] [kMultiCockDesc] a loving caress, urging [himher] on with soft words.", parse);
						Text.NL();
						Text.Add("The horny elf doesn't seem to need much encouragement. [HeShe] has a good length of [hisher] member in [hisher] mouth, awkwardly bobbing [hisher] head despite [hisher] strange position. Occasionally, [heshe] comes up for air, gasping before returning to [hisher] [kCockDesc] with renewed fervor.", parse);
					}
				}
				else {
					Text.Add("With your free hand, you start stroking [name]'s [kMultiCockDesc] rapidly. The elf mumbles a moaning thanks, before tumbling back into oblivion, [hisher] loins aflame with your combined assault.", parse);
				}
				Text.NL();
				Text.Add("Before long, you notice an erratic twitching in [name]'s [kMultiCockDesc]. Acting quickly, you slam your fingers into [hisher] [kAnusDesc], pushing them up to the knuckle. Each thrust jabs directly at the poor elf's prostate, eliciting cry after cry of intense pleasure.", parse);
				Text.NL();
				...
				if(cum > 3)
					Text.Add("The elf has way more cum stored up than [hisher] lithe frame has a right to. It's not long before [hisher] mouth, but also [hisher] face, [kHairDesc] and [kBreastDesc] are completely drenched in the stuff. [HeShe] gulps down as much of it as [heshe] can, if nothing else to clear [hisher] clogged windpipe.", parse);
				else
				...
				Text.Add("As your pumping fingers deal with [name]'s once-tight back door, you let your other hand play with [hisher] [kVagDesc]. The elf's pliant folds part easily, [hisher] passage already dripping with juices from your anal teasing.", parse);
				Text.NL();
				...
				if(kiakai.FirstVag().clitLength.Get() > 10) {
					...
					Text.Add("Relenting, you allow the elf time to recover, releasing [hisher] clit until [hisher] trembling subsides. Your other hand, however, doesn't let up its rhythmic reaming of [name]'s [kAnusDesc].", parse);
					Text.NL();
				...
				Text.NL();
				Text.Add("Done with [hisher] clit, you move your attention to [name]'s [kVagDesc]. No further preparation is needed, as your fingers easily slip inside [hisher] folds, lubricated by the elf's own fluids. Eagerly, you explore [hisher] sopping passage, coaxed on by [hisher] encouraging moans.", parse);
				Text.NL();
				Text.Add("After a while, you decide that you have sated the elf's feminine parts sufficiently, and return your focus to [hisher] [kAnusDesc]. Redoubling your efforts, it is only a matter of minutes before [name]'s confused sphincter convulses around your penetrating digits. The elf almost passes out as [hisher] anal orgasm shocks [himher] to [hisher] very core.", parse);
			}
	...
	options.push({ nameStr : "Denial",
		func : function() {
			...
			if(kiakai.FirstCock())
				Text.Add("You trail one finger down the length of the elf's [kMultiCockDesc], even placing a kiss on[oneof] [hisher] [kCockTip]", parse);
			else if(kiakai.FirstVag())
				Text.Add("You trail one finger down the elf's tight slit, even placing a kiss on [hisher] [kClitDesc]", parse);
			...
			if(kiakai.FirstCock())
				...
				Text.Add("The elf bites [hisher] lip, struggling to keep [himher]self from thrusting [hisher] moist crotch against your tantalizing [tongueDesc].", parse);
			Text.NL();
			...
			Text.NL();
			if(kiakai.FirstCock()) {
				Text.Add("Grasping [hisher] [kCockDesc] in one hand, [name] begins to stroke [himher]self rapidly. [HisHer] other hand snakes down between [hisher] legs, ", parse);
				if(kiakai.FirstVag())
				...
				Text.Add("Spreading [hisher] lower lips with one hand, [name] begins to finger [himher]self with the other, pushing [hisher] digits in up to the knuckles. Trembling slightly, [heshe] prods at [hisher] [kClitDesc] with [hisher] thumb, breathing heavily.", parse);
				Text.NL();
				Text.Add("It's not long before the elf cries out in pleasure, [hisher] [kVagDesc] twitching and squirting [hisher] juices on the ground.", parse);
			}
			Text.NL();
...
Scenes.Kiakai.HealingAssertive = function() {
		...
		options.push({ nameStr : "Blowjob",
			func : function() {
				...
				if(kiakai.flags["Sexed"] < 3) {
					...
					Text.NL();
					Text.Add("Uncertainly eyeing[oneof] your [multiCockDesc], [name] leans forward and gives it a tentative lick, placing a kiss on the crown. Sighing impatiently, you grasp the back of [hisher] head firmly, forcing the first few inches of your [cockDesc] into the protesting elf's hot mouth.", parse);
				}
				else if(kiakai.flags["Sexed"] < 10) {
					Text.Add("Nodding in comprehension, [name] swallows uncertainly. Grasping[oneof] your [multiCockDesc], the elf takes part of it into [hisher] mouth, lathering your length in saliva.", parse);
					Text.NL();
					...
					Text.NL();
					Text.Add("Time you took a more active part though. Grasping the back of [hisher] head with both hands, you push more of your [cockDesc] into [himher], glorying in [hisher] magnificent tightness.", parse);
				}
				Text.NL();
				Text.Add("Holding [himher], you guide the elf up and down your member, slowly at first, but gradually increasing in speed. [name]'s [kTongueDesc] caresses the veins of your [cockDesc], trying to keep up with your rapid pace.", parse);
				if(player.NumCocks() > 1) {
					parse["s"] = player.NumCocks() > 2 ? "s" : "";
					parse["notS"] = player.NumCocks() > 2 ? "" : "s";
					Text.Add(" Your other [cockDesc][s] grind[notS] against [name]'s face, rubbing [hisher] forehead and staining [hisher] hair with sticky pre-cum.", parse);
				}
				Text.NL();
				...
				scenes.AddEnc(function() {
					...
					Text.Add("[name] has some trouble getting [hisher] breath as your [cockDesc] stuffs [hisher] mouth to the brim, the [cockTip] prodding at the back of [hisher] throat. The elf tries to push [himher]self off your cock, but you grip [himher] firmly, restricting [hisher] movements.", parse);
					Text.NL();
					...
					if(kiakai.flags["Sexed"] > 20)
						...
						Text.Add("Not used to such rough treatment - yet - the elf's eyes tear up from pain and lack of oxygen. Still, [hisher] convulsing throat is working wonders on your [cockDesc], stroking it rapidly while fruitlessly trying to expel the invading member.", parse);
					Text.NL();
					...
					if(biggestCockLen - kiakai.flags["Sexed"] < 20)
						Text.Add("pushing your [cockDesc] in to the root. The hot kiss on your groin only serves to egg you on.", parse);
					else
						Text.Add("pushing as much of your [cockDesc] into [himher] as possible. It will take a lot of training until [heshe] is able to take all of your length, though.", parse);
					Text.NL();
					...
					if(player.NumCocks() > 1) {
						...
						Text.Add(" Joining [itsTheir] sibling, your other [cockDesc][s] erupt[notS], covering [name] in long white strands.", parse);
					}
					Text.NL();
					Text.Add("With a drawn-out sloppy sound, you pull your softening [cockDesc] from the elf's throat. Once [heshe] has finished coughing, a dribble of cum running down [hisher] chin, you present [himher] with your cum-coated cock.", parse);
					Text.NL();
				...
				scenes.AddEnc(function() {
					...
					if(biggestCockLen > 30)
						Text.Add("Due to the sheer size of your [cockDesc], you are unable to use its full length. Alternating between roughly deepthroating the elf and repeatedly forcing your way in and out of [hisher] throat, you make the most of it anyway.", parse);
						...
						Text.Add("Roughly smashing your crotch against [hisher] lips, you pummel the elf, making sure to familiarize [himher] with every inch of your [cockDesc].", parse);
					Text.NL();
					Text.Add("Hardly given time to breathe, [name]'s muffled moans grow more ragged. Not showing any signs of letting up, you continue your face-fucking for several minutes. In a small act of mercy, you pull out in order to let the elf recover.", parse);
					Text.NL();
					if(kiakai.flags["Sexed"] > 30)
						Text.Add("<i>“W-why did you stop?”</i> [name] coughs, almost sounding a bit disappointed. You tell [himher] not to worry, there is more where that came from. The elf is wearing an almost hungry expression as you once more guide your [cockDesc] in between [hisher] waiting lips.", parse);
					else
						Text.Add("<i>“Y-you are way too rough, [playername],”</i> [name] splutters, coughing up pre-cum. You chide [himher] for being such a baby - a good healer should be able to take this much at least, right? Cutting off any further complaints, you shove your [cockDesc] back inside the elf.", parse);
					Text.NL();
					...
					var scenes = new EncounterTable();
					scenes.AddEnc(function() {
						Text.Add("Deciding to push on and leave the elf with a surprise present, you continue thrusting your throbbing [cockDesc] into [hisher] mouth, hardly slowing down even as you begin to spill your seed inside [himher]. Part of your load seeps out of the corner of [name]'s mouth, dribbling down [hisher] chin.", parse);
						if(cum > 3)
					...
					scenes.AddEnc(function() {
						Text.Add("You pull your throbbing [cockDesc] away from the elf, excitedly jerking your length in front of [name]'s open, panting mouth. Still slightly disoriented from the rough fuck, [heshe] doesn't seem to notice, until the first wad lands on [hisher] [kTongueDesc].", parse);
						Text.NL();
				...
				*/
				...
				Text.NL();
				if(kiakai.flags["Sexed"] > 30) {
					Text.Add("<i>“Mm... y-you taste so good, [playername],”</i> [name] stammers, licking up a stray drop of your cum from your [cockDesc]. Chuckling, you tell the slutty elf to get dressed. Maybe next time, you'll let [himher] get off, too.", parse);
				}
				else if(kiakai.flags["Sexed"] > 10) {
					...
					Text.Add("<i>“P-please don't make me do that again, [playername],”</i> [name] pleads with you, <i>“I don't think I can take it.”</i> You smile at [himher] reassuringly, telling the elf that next time won't be so bad. [HeShe] visibly shudders when you mention a next time.", parse);
				}
				Text.NL();
	...
	if(player.FirstVag()) {
		options.push({ nameStr : "Cunnilingus",
			func : function() {
				Text.Clear();
				Text.Add("You flaunt your [vagDesc] for [name]'s benefit, slyly asking if [heshe] wants to eat you out.", parse);
				Text.NL();
				if(kiakai.flags["Sexed"] < 10)
				...
				Text.Add("Nodding impatiently, you guide [hisher] head down [lowerbody], positioning yourself within reach of the elf's [kTongueDesc]. Any lingering inhibitions shattered by your insistence, [name] leans in to pleasure you as best as [heshe] can.", parse);
				Text.NL();
				parse["bothtwo"] = player.Arms().count > 2 ? "two" : "both";
				Text.Add("You stifle a moan as you feel [hisher] lips on your [vagDesc], the tip of the elf's [kTongueDesc] hesitantly probing at your labia. Not patient enough to let [himher] go at [hisher] own pace, you grasp [name]'s head with [bothtwo] hands and push [himher] down roughly, your [vagDesc] grinding against [hisher] face.", parse);
				Text.NL();
				Text.Add("For a while, you just enjoy the sensation as [name] slowly accepts [hisher] position, starting to actively lap at your crotch, burying [hisher] [kTongueDesc] inside your folds. Your grip keeps the elf firmly in place, barely giving [himher] room to breathe.", parse);
				Text.NL();
				if(player.FirstCock()) {
					parse["isAre"] = player.NumCocks() > 1 ? "are" : "is";
					parse["itThem"] = player.NumCocks() > 1 ? "them" : "it";
					Text.Add("Your unattended [multiCockDesc] [isAre] starting to stiffen, and you idly slap [name] with [itThem] a few times. After smearing some of your pre-cum on [hisher] forehead, you let [himher] get back to business.", parse);
					Text.NL();
				}
				if(kiakai.flags["Sexed"] < 20)
				...
				scenes.AddEnc(function() {
					...
					var scenes = new EncounterTable();
					scenes.AddEnc(function() {
						Text.Add("<i>“Mm... not bad, but perhaps you still need some guidance...”</i>", parse);
					}, 1.0, function() { return true; });
					scenes.AddEnc(function() {
					...
					parse["Y"] = player.HasLegs() ? "Spreading your legs wider, y" : "Y";
					Text.Add(" To accentuate your words, you grind up against [hisher] mouth, pressing down on [himher] with your [vagDesc]. [Y]ou let yourself relax, putting more and more of your weight on your prone companion. [name]'s muffled moans send violent vibrations through your nether regions, [hisher] [kTongueDesc] firmly lodged in your slick tunnel.", parse);
					
					if(player.FirstCock()) {
						parse["itThem"] = player.NumCocks() > 1 ? "them" : "it";
						Text.Add(" You grab hold of your [multiCockDesc], stroking [itThem] slowly while riding the submissive elf.", parse);
					}
					var tail = player.HasTail();
					if(tail && tail.Prehensile()) {
						parse["notEs"] = tail.count > 1 ? "" : "es";
						Text.Add(" Your flexible [tailDesc] swish[notEs] back and forth, brushing over [name]'s stomach before finding its target and rubbing up against the elf's [kGenDesc], returning blow for blow.", parse);
					}
					else if(tail) {
						parse["notEs"] = tail.count > 1 ? "" : "es";
						parse["itsTheir"] = tail.count > 1 ? "their" : "its";
						Text.Add(" [name] shudders as your [tailDesc] brush[notEs] against [hisher] stomach, [itsTheir] sweeping motions tickling [hisher] sensitive [kSkinDesc].", parse);
					}
					Text.NL();
	...
	options.push({ nameStr : "69",
		func : function() {
			...
			if(kiakai.FirstCock() && kiakai.FirstVag()) {
				...
				Text.NL();
				Text.Add("While giving [himher] a few strokes on[oneof] [hisher] member[s] to get [himher] riled up, your other hand playfully pinches one of the elf's [kNipsDesc].", parse);
			}
			else if(kiakai.FirstVag()) {
			...
			if(player.FirstCock()) {
				parse["s"] = player.NumCocks() > 1 ? "s" : "";
				Text.Add("Letting your [multiCockDesc] hover a hair's breadth in front of the elf's open mouth, you enjoy the feeling of [hisher] rapid breathing on the stiffening length[s].", parse);
				Text.NL();
			}
			...
			if(kiakai.FirstCock()) {
				options.push({ nameStr : "Cock",
					func : function() {
						...
						Text.NL();
						Text.Add("Without missing a beat, you begin to drive [name] toward [hisher] next climax. This time, you don't play around, going straight for the kill. Ramming as much as you can of the elf's [kCockDesc] into your mouth, you go at it with great abandon. Your [tongueDesc] grinds the undercarriage of the [thickThin] shaft, occasionally unsuccessful in trying to force its way into [name]'s urethra.", parse);
						Text.NL();
						...
						options.push({ nameStr : "Deny",
							func : function() {
								...
								Text.NL();
								Text.Add("<i>“You'll cum when I allow you to, not before,”</i> you counter, letting your [tongueDesc] play along the tip of the elf's cock. Powerless to resist, [name] falls back, letting you do as you please.", parse);
								Text.NL();
				...
				options.push({ nameStr : "Vagina",
					func : function() {
						...
						if(player.LongTongue()) {
							...
							Text.NL();
							Text.Add("Taste buds all along your engorged organ sample [name]'s clear liquids, and you hungrily slurp them up as you thrust your [tongueDesc] into the willing elf. Without warning, [hisher] tunnel clamps down around you, and you feel the elf spasm beneath you.", parse);
						}
						else {
						...
						Gui.NextPrompt(function() {
							...
							Text.NL();
							Text.Add("You aren't done with your little elf just yet though. No sooner has [heshe] finished trembling from [hisher] last climax, you jump right back in, this time using both your hands and your [tongueDesc] to drive [himher] toward another orgasm.", parse);
							Text.NL();
			...
			options.push({ nameStr : "Anal",
				func : function() {
					...
					Text.NL();
					Text.Add("You gyrate your [hipsDesc] slowly, pressing your [genDesc] against [name], urging [himher] to fulfill [hisher] end of the bargain. Without pause, you continue to focus on the elf's butt, licking the orifice meticulously clean and teasing [himher] mercilessly.", parse);
					Text.NL();
					...
					if(player.LongTongue()) {
						options.push({ nameStr : "Tongue",
							func : function() {
								...
								Text.Clear();
								Text.Add("Your [tongueDesc], rival to a cock in length, snakes out and buries itself deep inside [name], immediately beginning to thrust in and out.", parse);
								if(kiakai.FirstCock())
									Text.Add(" Due to your size, you can easily reach [hisher] prostate, each thrust mashing your [tongueDesc] against the sensitive elf's sensitive pleasure button.", parse);
								PrintDefaultOptions();
							}, enabled : true,
	...
	options.push({ nameStr : "Anal",
		func : function() {
			...
			if(kiakai.flags["AnalExp"] < 2) {
				Text.Add("<i>“Like what you see?”</i> you tease the prone elf, shuffling your hips slightly, bringing your [genDesc] closer to [hisher] face. [name]'s face is bright red at this point, unable to tear [hisher] eyes off you.", parse);
				Text.NL();
				...
				Text.Add("Mumbling, [name] assures you that [heshe] is fine, avoiding your gaze, but you'll have none of it. <i>“Can't have the healer getting a fever, can we?”</i> You instruct your companion to lay down across your lap, face down. A bit doubtful, the elf complies to your wishes. [HeShe] shudders slightly as [hisher] [kGenDesc] accidentally brush against your [leg]. Apologetically, [heshe] shifts around, raising [hisher] butt to break the contact. Coincidentally, this suits your intentions perfectly.", parse);
				Text.NL();
				...
				Text.Add("Deftly pumping your digits in and out of the quivering elf, you commend [himher] for being such a good [boygirl]. [name]'s only response is a whorish moan as you rail [hisher] [kAnusDesc].", parse);
				Text.NL();
			...
			// TODO: ADD TOYS SCENE
				...
				Text.Add("<i>“Now then...”</i> in a sultry voice, you coax [name] to shuffle around in your lap, presenting you with [hisher] butt. Knowing what to expect, the elf shudders in pleasure as you insert your first digit into [hisher] [kAnusDesc].", parse);
				if(numFingers > 1) {
					Text.NL();
				...
				if(numFingers < 5) {
					...
					Text.NL();
					Text.Add("<i>“Oh, you seem to be <b>very</b> healthy indeed,”</i> you congratulate your companion, one hand busy caressing [hisher] silky hair, the other deep inside [hisher] [kAnusDesc].", parse);
					Text.NL();
					Text.Add("Increasing your pace, you bring the horny elf to a shuddering climax", parse);
					
					parse["legs"] = player.LegsDesc();
					parse["kleg"] = kiakai.LegDesc();
					
					if(kiakai.FirstCock())
						Text.Add(", [hisher] [kCockDesc] depositing its load on your [legs]", parse);
					if(kiakai.FirstVag())
						Text.Add(", a trickle of feminine juices trailing down [hisher] [kleg] from [hisher] [kVagDesc]", parse);
					Text.Add(".", parse);
				}
				else {
					Text.Add("<i>“You asked for it, horny little elf,”</i> you mutter under your breath, slowly pushing more and more of your hand into [hisher] [kAnusDesc]. [name] cries out loudly - if in pleasure or pain you cannot tell - as the widest part of your hand suddenly pops past [hisher] formerly tight ring.", parse);
					Text.NL();
					Text.Add("Almost surprised yourself, you chuckle as you allow the elf some time to relax before exploring the extent of your newfound reach. You can push a fair bit of your arm up [hisher] [kAnusDesc] before encountering resistance.", parse);
					Text.NL();
					Text.Add("<i>“Mm, seems I have trained you well, [name],”</i> you tease the elf as your fist wreaks havoc on [hisher] insides.", parse);
					Text.NL();
					parse["thighs"] = player.ThighsDesc();
					
					if(kiakai.FirstCock()) {
						Text.Add("Settling to a halt far inside, you cup your hand, massaging [hisher] prostate. This almost immediately causes the delirious elf to launch into orgasm, [hisher] [kMultiCockDesc] painting sticky trails across your [thighs].", parse);
					}
					else {
						Text.Add("Taking care not to harm [himher], you slowly pump your arm in and out of [name]. The elf seems to be very happy with your treatment, begging for you to go faster, go deeper. Shuddering, [hisher] [kVagDesc] finally gives in, leaking girly fluids across your [thighs].", parse);
					}
				}
			...
			options.push({ nameStr : "Rough",
				func : function() {
					...
					if(kiakai.FirstCock() && Math.random() < 0.5) {
						...
						Text.NL();
						Text.Add("It's not long before the elf's prostate has had enough, and its load explodes from [hisher] [kMultiCockDesc], some of it splattering against [name]'s chest, before slowly forming a pool on the ground beneath [himher].", parse);
						Text.NL();
					}
					...
					Text.NL();
					Text.Add("Plunging in and out, your pile of hot meat pummels the tight elf beneath you. Your elven cocksleeve goes through moaning, begging for more and whimpering for you to slow down. Eventually, [hisher] arms give and [heshe] collapses to the ground, only suspended by your [cockDesc] and your hands on [hisher] [kHipsDesc]. All of [name]'s pleading falls on deaf ears as you keep up your even, incessant pounding throughout [hisher] moaning.", parse);
					Text.NL();
				...
				options.push({ nameStr : "Ruin",
					func : function() {
						...
						Gui.NextPrompt(function() {
							...
							if(player.FirstCock()) {
								Text.Add("Your mindless rutting eventually rouses a familiar feeling in your [ballsDesc] as you sense the oncoming wave of your orgasm. [name] gasps as [heshe] feels the first glob of hot sperm land in [hisher] ravaged colon. Keeping up your steam, you slam into the elf another dozen times, bottoming out while unloading inside [himher].", parse);
								Text.NL();
		...
		func : function() {
			...
			if(attitude) {
				if(attitude == Kiakai.SexFirstAttitude.Passive) {
					...
					Text.NL();
					Text.Add("As if [hisher] inhibitions are lifted by your urgings, you almost immediately feel one of [name]'s slender fingers sneak down your crack, prodding at the entrance of your [anusDesc]. The elf is gentle, but makes no effort to hide [hisher] eagerness as [heshe] plunges two digits into your yearning sphincter.", parse);
					Text.NL();
					Text.Add("Unsated, you dive in, stealing a series of steamy kisses from the breathless elf. You let your [tongueDesc] entwine with [hishers], losing yourself in oral pleasure, leaving [name] free rein of your [anusDesc]. [HeShe] uses the fingers of both hands to spread you open, stretching your ring before pushing the slender digits in as far as the awkward angle will allow.", parse);
					Text.NL();
					...
					Text.NL();
					Text.Add("<i>“Nice and tight, isn't it?”</i> you murmur huskily, <i>“wouldn't you like to bury [thatThose] [kMultiCockDesc] of yours there?”</i> [name] looks shocked when [heshe] realizes what you are offering... nay, ordering.", parse);
					Text.NL();
					if(kiakai.FirstCock()) {
						Text.Add("[name]'s [kMultiCockDesc] [isAre] rising to the challenge, stiff between [hisher] legs as the excited elf continues to finger you.", parse);
						if(player.LowerBodyType() != LowerBodyType.Single) {
							Text.NL();
							Text.Add("<i>“Getting excited, aren't we?”</i> you taunt, placing your foot on[oneof] [hisher] engorged member[s]. Grinning, you let [name] take care of your [anusDesc], playing with yourself in the meantime. You slowly increase the pressure on the elf's [kCockDesc], which has become painfully hard, judging from [name]'s groans.", parse);
							kiakai.AddLustFraction(0.3);
						}
					}
					Text.NL();
			...
			scenes.AddEnc(function() {
				...
				scenes.AddEnc(function() {
					Text.Add("<i>“What if I did it myself?”</i> The elf waves at[oneof] [hisher] [kCockDesc]. <i>“It is quite big, so...”</i> [heshe] trails off, embarrassed.", parse);
					Scenes.Kiakai.AnalCatchPrep(Kiakai.AnalCatchPrepScene.Selfsuck);
				}, 1.0, function() { return kiaiCock.length.Get() >= 25; });
				scenes.AddEnc(function() {
					Text.Add("<i>“P-perhaps you can provide some?”</i> the elf stutters. When you raise your eyebrow quizzically, [heshe] gestures toward your [multiCockDesc], embarrassed.", parse);
...
Scenes.Kiakai.AnalCatchPrep = function(choice, assert) {
	...
	if     (choice == Kiakai.AnalCatchPrepScene.Rim) {
		...
		Text.Add("<i>“Just lean back and relax...”</i> [name] murmurs[legs]. Gently, the elf spreads your cheeks, planting a kiss directly on your yearning rosebud. [name] plies you open as [heshe] teases your opening with [hisher] [kTongueDesc], licking and lapping before plunging in.", parse);
		Text.NL();
		...
		Text.Add("Wordlessly, [name] parts [hisher] legs, giving you full access to [hisher] [kMultiCockDesc]. Smiling, you lean in and plant a kiss on[oneof] [hisher] [kCockTip][s]. Eager to get to the main course, you waste no time in ramming as much of the [kCockDesc] as you can down your throat, lathering its length in slick saliva.", parse);
		Text.NL();
		if(kiakai.FirstCock()) {
		...
		Text.Add("[name] gives [himher]self a few experimental strokes, coaxing[oneof] [hisher] [kCockDesc] to rise to its full glory. Nervously, the elf dips [hisher] head, shuddering as [heshe] plants a kiss on the twitching [kCockTip].", parse);
		Text.NL();
		if(kiakai.FirstCock()) {
		...
		Text.NL();
		Text.Add("<i>“A-are you ready to go, [playername]?”</i> the elf huffs when your fountain has finally dried up. While you just got off, your companion is still aching to fuck, [kCockDesc] stiff and eager. Weakly, you nod to [himher], urging the elf to go give in to [hisher] desire.", parse);
	}
	else if(choice == Kiakai.AnalCatchPrepScene.Oil) {
		party.inventory.RemoveItem(Items.SnakeOil);
		Text.Add("You produce a vial of slick oil from your bags. The liquid is cool on your [skinDesc] as the elf generously applies it to your [anusDesc], working some of the sticky substance into your passage. [name] pours the rest on[oneof] [hisher] [kMultiCockDesc], spreading the soothing lubricant with long, slow strokes.", parse);
		Text.NL();
	...
	Text.NL();
	if(len > cap)
		Text.Add("You moan as the [kCockTip] of the elf's [kCockDesc] prods at the deepest reaches of your colon. A glorious feeling of fullness spreads through your nethers as the large shaft strains against your limits. Your only regret is that you can't take it all.", parse);
	else
	...
	if(player.FirstCock()) {
		...
		parse["s"]        = player.NumCocks() > 1 ? "s"     : "";
		Text.Add(" Your own [multiCockDesc] [isAre] straining to get free, trapped against [name]'s [kStomachDesc] as pre-cum leaks in generous amounts from [itsTheir] tip[s].", parse);
	}
	if(kiakai.HasBalls())
	...
	options.push({ nameStr : "Take charge",
		func : function() {
			...
			Text.Add("Time to give the elf a good workout. Without any regard for [hisher] feelings, you use [himher] as a living sex toy, repeatedly impaling yourself on [hisher] [kCockDesc]. Hissing, you repeat your command that [heshe] is not allowed to cum until the moment you tell [himher] so, not under any circumstances.", parse);
			Text.NL();
			
			if(tailpegFlag) {
				Text.Add("Not that you are by any means making that easy for [himher]. In addition to your [anusDesc] constricting around [hisher] [kCockDesc], your tail is shoved so far up [hisher] [kAnusDesc] that it's unlikely the elf will be able to sit for quite some time. With each bounce, the appendage squirms inside [name]", parse);
				...
				Text.Add("[name] is quite the multitasker, as [heshe] hasn't neglected your [cockDesc] for even a second, no matter how rough the ride is getting. [HeShe] isn't doing that bad either, considering the circumstances. Still, you could never miss a chance to tease [himher].", parse);
				Text.NL();
				...
				Text.Add("Grinning, you grasp at the base of the [kCockDesc2], clasping your fingers around it painfully tightly[thk]. Keeping your grip tight, you begin to jerk it again, your vice-like grip making sure that no stray splash of pre escapes the throbbing shaft.", parse);
				Text.NL();
			}
			...
			// You cum first
				...
				Text.Add("Despite your mocking bravado, you are the first one to cum, crying out as you feel your climax wreck your body. Judging by the way that [name]'s [kCockDesc] is twitching in the crushing confines of your [anusDesc], [heshe] probably won't be far behind.", parse);
				var cum = player.OrgasmCum();
				if(player.FirstVag())
	...
	options.push({ nameStr : "More",
		func : function() {
			...
			scenes.AddEnc(function() {
				...
				Text.Add("You lie back and enjoy your companion's relentless pounding of your [anusDesc], occasionally playing with yourself, or [br] [name]’s [kBreastDesc].", parse);
				Text.NL();
			...
			if(kiakai.FirstCock()) {
				Text.Add("The elf’s hot seed pours into your convulsing [anusDesc], your tight ring milking [himher] of [hisher] spunk.", parse);
			...
			if(player.FirstCock()) {
				...
				Text.Add(" [name]’s [kCockDesc] is roughly jammed against your prostate, pushing all the right buttons. You blow your load, [mess] sticky mess on both of your bodies.", parse);
				if(cum > 3)
			...
			if(player.FirstVag())
				Text.Add(" Clear juices flow from your needy [vagDesc]. While it received no action this time, the rough loving the elf provided your ass with was more than enough to push you over the edge.", parse);
			Text.NL();
	...
	options.push({ nameStr : "Beg",
		func : function() {
			...
			if(cockFlag) {
				...
				Text.NL();
				Text.Add("Hands trembling, you withdraw from your bobbing [cockDesc]. Smiling beneficently, [name] grasps the twitching member with [hisher] slender fingers, sending a shiver up your spine. The means of your pleasure taken out of your hands, all you can do is lie back and enjoy [name]'s ministrations, [hisher] pumping hand tightly grasping your needy [cockDesc], [hisher] own [kCockDesc] rapidly thrusting into your [anusDesc].", parse);
			}
			else if(vagFlag) {
				...
				Text.NL();
				Text.Add("With the means of your pleasure taken out of your hands, all you can do is lie back and enjoy [name]'s ministrations as [hisher] hand explores your needy cunt and [hisher] [kCockDesc] rapidly thrusts into your [anusDesc].", parse);
			}
			else if(breastFlag) {
			...
			Gui.NextPrompt(function() {
				...
				Text.Add("<i>“Y-yes, I love it when you take charge!”</i> You moan, biting your lip as [name]'s hot shaft pummels your back passage[lube]. Growing bolder by the minute, the elf kneads your [buttDesc], roughly manhandling you.", parse);
				Text.NL();
				...
				if(kiakai.FirstCock()) {
					...
					options.push({ nameStr : "Inside",
						func : function() {
							Text.Clear();
							Text.Add("<i>“I-inside!”</i> You moan, begging to be filled with hot cream. It's unclear whether the elf obeys you, or if [heshe] is simply too far gone to care. [name] cries out, [hisher] pulsating [kCockDesc] pouring its contents directly into your receptive bowels.", parse);
							var cum = kiakai.OrgasmCum();
							if(cum > 3)
