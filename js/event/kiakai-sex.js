...
Scenes.Kiakai.HealingSeducePrompt = function() {
	...
	var parse = {
		...
		vag      : function() { return player.FirstVag().Short(); },
		armor        : function() { return kiakai.ArmorDesc(); },
		hair     : function() { return kiakai.Hair().Short(); }
	};
	...
	options.push({ nameStr : "Seduce",
		func : function() {
			...
			Text.NL();
			Text.Add("A jolt runs up the elf's spine as you languidly caress [hisher] [hair], whispering in [hisher] ear that [heshe] should get out of those pesky clothes.", parse);
			Text.NL();
			if(kiakai.flags["Sexed"] > 10)
				...
				options.push({ nameStr : "Passive",
					func : function() {
						...
						*/
						...
						scenes.AddEnc(function() {
							if(kiakai.flags["Sexed"] > 10) {
								Text.Add("The sudden tightening of your [cock] gives the elf all the warning needed. [HeShe] begins to deep-throat you repeatedly, massaging you with the muscles at the back of [hisher] throat. Moaning loudly, you give [name] what [heshe] wants: a generous serving of hot spunk, delivered directly from the tap. ", parse);
								var cum = player.OrgasmCum();
	...
	if(player.FirstVag()) {
		...
		options.push({ nameStr : "Eat you",
			func : function() {
				...
				Text.NL();
				Text.Add("You close your eyes in bliss as you feel [name] dig in, [hisher] tongue tasting the wet folds of your [vag]. Urged on by your soft sighs, the elf continues to lap away at your moistness. Trying [hisher] best to please, [name]'s deft [kTongueDesc] finds your [clit].", parse);
				Text.NL();
				...
				if(player.FirstVag().clitCock) {
					...
					if(player.HasBalls())
						Text.Add(" As [hisher] [kTongueDesc] returns to work on your feminine parts, [name] caresses your [balls], sending shivers up your spine.", parse);
				}
				else if(kiakai.flags["Sexed"] > 15) {
				...
				scenes.AddEnc(function() {
					...
					// Regular fingering
						...
						Text.Add("Finally, with a shuddering moan, you cum. Your [legs] squirm as you shake uncontrollably, gushing sweet nectar all around your lover's fingers.", parse);
						if(player.FirstCock()) {
							parse["s"] = player.NumCocks() > 1 ? "" : "s";
				...
				scenes.AddEnc(function() {
					...
					Text.NL();
					Text.Add("More than pleased with the dual attention you are given, you decide to help out, grabbing[oneof] your [cocks] and heartily jerking yourself off while the elf administers [hisher] healing skills on your [vag]. Before long, your trembling [legs] announce your approaching orgasm.", parse);
					Text.NL();
					if(kiakai.flags["Sexed"] > 15 && Math.random() > 0.5) {
						...
						Text.Add("Slightly flustered, [name] tries to focus on cleaning your [vag], trying to ignore the white strands dripping down your cock[s], landing on [hisher] nose.", parse);
					}
					Text.Flush();
					Gui.NextPrompt();
				}, 1.0, function() { return player.FirstCock(); });
				scenes.AddEnc(function() {
					Text.Add("By this time, [name] knows just what to do to get you off. Electric shocks run through your tingling labia as [heshe] administers a spark of healing energy with the tip of [hisher] [kTongueDesc]. The sparks quickly home in on the most prominent feature of your feminine anatomy, your [clit].", parse);
					Text.NL();
					Text.Add("The elf isn't even touching you anymore; [heshe] is just letting [hisher] healing power do the work. [HeShe] lets [hisher] [kTongueDesc] hover a hair's breadth away from your [vag], a tiny beam of refreshing energy bridging the gap between the two of you.", parse);
					Text.NL();
					...
					if(player.FirstCock()) {
						parse["s"] = player.NumCocks() == 1 ? "s" : "";
						Text.Add("Your [cocks] erupt[s], covering [name]'s hair and back with sticky fluids.", parse);
	...
	if(kiakai.FirstCock()) {
		...
		options.push({ nameStr : "Blowjob",
			func : function() {
				...
				Text.NL();
				Text.Add("Slowly, you inch [name]'s [kCockDesc] into your mouth, coating the moaning elf's length in your slick saliva. Letting your [tongue] stroke the stem playfully, you start bobbing your head up and down, eagerly swallowing your companion's trembling dick.", parse);
				Text.NL();
				if(kiakai.flags["Sexed"] < 10)
				...
				if(kiakai.FirstCock().length.Get() > 35) {
					...
					Text.Add("A cock of this size is no problem for you to devour whole. You lean forward, planting kiss after kiss on [name]'s crotch, letting your [tongue] work on every part of [hisher] [kCockDesc]. Alternating your focus between licking on the [kCockTip] and taking [himher] to the root, you make sure to give [name] as good a time as you are able.", parse);
				}
				Text.NL();
				Text.Add("<i>“B-by Aria, [playername], that feels amazing!”</i> the elf moans. You surface for a moment, a strand of saliva still connecting your [tongue] with [hisher] [kCockDesc]. Flushed and panting, you manage to express that your diligent healer deserves no less, before returning to the task at hand.", parse);
				Text.NL();
	...
	if(kiakai.FirstVag()) {
		...
		options.push({ nameStr : "Eat out",
			func : function() {
				...
				Text.NL();
				Text.Add("Done with foreplay, you let your [tongue] plunge into [name], eliciting a loud yelp from the unprepared elf. As you thrust deeply into [hisher] [kVagDesc], [name] offers cute, encouraging moans, wordlessly egging you on.", parse);
				Text.NL();
				...
				Text.NL();
				Text.Add("[name] cannot take your ministrations for long, [hisher] legs twitching slightly as [heshe] lets [himher]self be overcome by lust. The elf cries out, [hisher] [kVagDesc] clamping down tight around your [tongue], as wave after wave of [hisher] orgasm rolls over you. Your taste buds are assaulted by [hisher] sweet overflowing juices, the excess dripping down your chin.", parse);
				Text.NL();
	...
	if(player.FirstCock()) {
		options.push({ nameStr : "Blowjob",
			func : function() {
				...
				Text.NL();
				Text.Add("Holding [himher], you guide the elf up and down your member, slowly at first, but gradually increasing in speed. [name]'s [kTongueDesc] caresses the veins of your [cock], trying to keep up with your rapid pace.", parse);
				if(player.NumCocks() > 1) {
					parse["s"] = player.NumCocks() > 2 ? "s" : "";
					parse["notS"] = player.NumCocks() > 2 ? "" : "s";
					Text.Add(" Your other [cock][s] grind[notS] against [name]'s face, rubbing [hisher] forehead and staining [hisher] hair with sticky pre-cum.", parse);
				}
				Text.NL();
	...
	if(player.FirstVag()) {
		options.push({ nameStr : "Cunnilingus",
			func : function() {
				...
				Text.Add("You stifle a moan as you feel [hisher] lips on your [vag], the tip of the elf's [kTongueDesc] hesitantly probing at your labia. Not patient enough to let [himher] go at [hisher] own pace, you grasp [name]'s head with [bothtwo] hands and push [himher] down roughly, your [vag] grinding against [hisher] face.", parse);
				Text.NL();
				...
				var scenes = new EncounterTable();
				scenes.AddEnc(function() {
					Text.Add("Shaking your head in disappointment, you push [name] back. The elf looks confused, wondering what [heshe] did wrong. <i>“[playername]?”</i> [heshe] asks uncertainly. You push [himher] down on [hisher] back, silencing the elf with a deep kiss. As you withdraw, a string of saliva mixed with your own juices briefly bridges the gap between [name]'s mouth and yours. Not allowing [name] to get back on [hisher] feet, you quickly straddle [hisher] face, your [butt] resting lightly on [hisher] [kBreastDesc].", parse);
					Text.NL();
					...
					Text.Add(" To accentuate your words, you grind up against [hisher] mouth, pressing down on [himher] with your [vag]. [Y]ou let yourself relax, putting more and more of your weight on your prone companion. [name]'s muffled moans send violent vibrations through your nether regions, [hisher] [kTongueDesc] firmly lodged in your slick tunnel.", parse);
					...
					if(tail && tail.Prehensile()) {
						...
						Text.Add(" Your flexible [tail] swish[notEs] back and forth, brushing over [name]'s stomach before finding its target and rubbing up against the elf's [kGenDesc], returning blow for blow.", parse);
					}
					else if(tail) {
						parse["notEs"] = tail.count > 1 ? "" : "es";
						parse["itsTheir"] = tail.count > 1 ? "their" : "its";
						Text.Add(" [name] shudders as your [tail] brush[notEs] against [hisher] stomach, [itsTheir] sweeping motions tickling [hisher] sensitive [kSkinDesc].", parse);
					}
					Text.NL();
		...
		func : function() {
			...
			var KIAI1 = function() {
				if(player.FirstCock()) {
					...
					Text.Add("The elf groans under your cruel teasing, [hisher] muffled complaints sending vibrations through your nethers. [HisHer] [kTongueDesc] is hard at work lapping at your [vag], worshipping your feminine sex.", parse);
					Sex.Cunnilingus(kiakai, player);
					kiakai.Fuck(null, 1);
					player.Fuck(null, 1);
				}
			...
			var KIAI2 = function() {
				...
				if(player.FirstCock()) {
					...
					Text.Add("The feeling of [name]'s [kTongueDesc] buried within your [vag] finally becomes too much for you, and you convulse against [hisher] face, coating [himher] with your liquids.", parse);
				}
			};
			...
			if(kiakai.FirstCock()) {
				options.push({ nameStr : "Cock",
					func : function() {
						...
						Text.NL();
						Text.Add("Without missing a beat, you begin to drive [name] toward [hisher] next climax. This time, you don't play around, going straight for the kill. Ramming as much as you can of the elf's [kCockDesc] into your mouth, you go at it with great abandon. Your [tongue] grinds the undercarriage of the [thickThin] shaft, occasionally unsuccessful in trying to force its way into [name]'s urethra.", parse);
						Text.NL();
			...
			if(kiakai.FirstVag()) {
				options.push({ nameStr : "Vagina",
					func : function() {
						...
						Text.Add("With your fingers, you spread the elf's labia, revealing the glistening pink flesh within. You confidently dig in, tasting your companion's most private parts. Your [tongue] circles the outer lips several times before moving on to your main course, [name]'s inviting cleft. Reaching in deeply, you jam your [tongue] inside [himher], lapping up [hisher] fluids gleefully.", parse);
						Text.NL();
						...
						if(player.LongTongue()) {
							...
							Text.Add("You are hard pressed to resist such a pretty gift, and greedily dig in. Slurping up any stray liquids trickling from the inviting slit, you let your [tongue] play a bit with [name]'s [kVagDesc] before focusing on [hisher] [kClitDesc].", parse);
							Text.NL();
							Text.Add("After preparing the elf with kisses, licks, and nibs, you envelop [hisher] [kClitDesc] completely, taking it into your mouth. Ever so carefully, you gently bite at the sensitive nub, sending sparks of electricity racing straight up the surprised elf's spine. Overwhelmed by the pleasure, [heshe] cries out, [hisher] back arching, pressing [himher] against your [breasts].", parse);
						}
						Text.NL();
			...
			options.push({ nameStr : "Anal",
				func : function() {
					...
					Text.Add("Switching gears, you use fingers from both hands to spread [name]'s asshole, opening the pink rosebud for you like a blooming flower. Leaning in, you let your [tongue] rake across [hisher] sensitive [cuntTaint] before finding its target, boring deep inside [name]'s [kAnusDesc].", parse);
					Text.NL();
					Text.Add("You gyrate your [hips] slowly, pressing your [genDesc] against [name], urging [himher] to fulfill [hisher] end of the bargain. Without pause, you continue to focus on the elf's butt, licking the orifice meticulously clean and teasing [himher] mercilessly.", parse);
					Text.NL();
					...
					if(player.LongTongue()) {
						options.push({ nameStr : "Tongue",
							func : function() {
								parse["penDesc"] = function() { return Text.Parse("your [tongue]", parse); };
								Text.Clear();
					...
					Gui.Callstack.push(function() {
						Text.NL();
						Text.Add("It's not long before [name]'s filled [kAnusDesc] constricts, and the elf's entire body shakes in pleasure. With a nudge of your [hips], you remind [himher] to return the favor. [HeShe] meekly complies, going down on your nethers with renewed vigor.", parse);
						Text.NL();
...
Scenes.Kiakai.Sex = function() {
	...
	var parse = {
		...
		breasts   : function() { return player.FirstBreastRow().Short(); },
		nips     : function() { return player.FirstBreastRow().NipsShort(); },
		cock     : function() { return playerCock.Short(); },
	...
	if(kiakai.flags["TalkedSex"] == 0) {
		...
		options.push({ nameStr : "Love",
			func : function() {
				...
				Text.NL();
				Text.Add("You pull [name] into a tight embrace, hugging [himher] close. Your hands trace down [hisher] spine, caressing [hisher] [kButtDesc]. The elf, who is pressed tightly against your [breasts], gives a surprised yelp. You whisper in [hisher] ear that you are there for [himher], that you want [himher], right here, right now.", parse);
				Text.NL();
...
Scenes.Kiakai.SexPrompt = function(attitude) {
	...
	var parse = {
		...
		cockTip      : function() { return playerCock.TipShort(); },
		knot     : function() { return playerCock.KnotShort(); },
		balls    : function() { return player.BallsDesc(); },
		tongue   : function() { return player.TongueDesc(); },
		butt     : function() { return player.Butt().Short(); },
		cocks: function() { return player.MultiCockDesc(); },
		...
		func : function() {
			...
			if(attitude) {
				if(attitude == Kiakai.SexFirstAttitude.Passive) {
					...
					Text.NL();
					Text.Add("Not trusting [himher]self to speak anymore, the elf merely whimpers as [heshe] adds another two fingers. You enjoy the display for a while, before announcing that you are moving on to the main course. Without waiting for a reply, you flip [name] over on all fours, nudging [hisher] legs apart, exposing [hisher] [kAnusDesc] to your eager [cock].", parse);
				}
			}
			// Repeats
			else {
			...
			if(kiakai.flags["SexPitchAnal"] == 0) {
				...
				Text.NL();
				Text.Add("<i>“I'll take it slow and gentle,”</i> you promise reassuringly. While talking, you have lubed up the length of your [cock], preparing it for entry. Humming softly, you place the [cockTip] against [name]'s [kAnusDesc], applying pressure. Already, you can feel [hisher] sphincter expanding grudgingly.", parse);
				Text.NL();
				...
				if(len < 15) {
					Text.Add("<i>“That wasn't so bad, was it?”</i> you comfort [name] as your [hips] lightly tap against [hisher] [kButtDesc], your [cock] bottoming out inside the elf.", parse);
					Text.NL();
					...
					Text.NL();
					Text.Add("You beg to differ, and set out to prove [himher] wrong. Although there are some noises of protest, soon enough your [hips] connect with [hisher] [kButtDesc], your [cock] hilted inside of [name]'s [kAnusDesc].", parse);
				}
				else {
				...
				Text.Add("[name]'s [kAnusDesc] is almost inviting, as if welcoming a returning guest. It takes surprisingly little effort for your [cock] to gain entry, pushing past the elf's weak defenses.", parse);
				Text.NL();
				...
				if(len < 20)
					Text.Add("Your [cock] easily bottoms out in your groaning companion, joining the two of you at the hip[balls].", parse);
				else if(len < 35)
			...
			options.push({ nameStr : "Gentle",
				func : function() {
					...
					Text.Add("You let your hips do the talking, letting your [cock] pump in and out of [name]'s [kAnusDesc]. If the elf had ever had doubts as to liking this, they seem to be all gone now.", parse);
					Text.NL();
					...
					Gui.NextPrompt(function() {
						...
						parse["cum"] = player.FirstCock() ? Text.Parse(", a trail of white fluid connecting the tip of your [cock] to [hisher] [kAnusDesc]", parse) : "";
			...
			options.push({ nameStr : "Rough",
				func : function() {
					...
					Text.NL();
					Text.Add("Plunging in and out, your pile of hot meat pummels the tight elf beneath you. Your elven cocksleeve goes through moaning, begging for more and whimpering for you to slow down. Eventually, [hisher] arms give and [heshe] collapses to the ground, only suspended by your [cock] and your hands on [hisher] [kHipsDesc]. All of [name]'s pleading falls on deaf ears as you keep up your even, incessant pounding throughout [hisher] moaning.", parse);
					Text.NL();
					...
					Gui.NextPrompt(function() {
						...
						if(player.FirstCock()) {
							...
							if(playerCock.knot && len < 35) {
								Text.Add("Not a single drop escapes from [name]'s ravaged hole as your [knot] seals [hisher] [kAnusDesc] shut. Seems like you will be here for a while.", parse);
	...
	options.push({ nameStr : "Anal catch",
		func : function() {
			...
			if(attitude) {
				if(attitude == Kiakai.SexFirstAttitude.Passive) {
					...
					if(kiakai.FirstCock()) {
						...
						if(player.LowerBodyType() != LowerBodyType.Single) {
							Text.NL();
							Text.Add("<i>“Getting excited, aren't we?”</i> you taunt, placing your foot on[oneof] [hisher] engorged member[s]. Grinning, you let [name] take care of your [anus], playing with yourself in the meantime. You slowly increase the pressure on the elf's [kCockDesc], which has become painfully hard, judging from [name]'s groans.", parse);
							kiakai.AddLustFraction(0.3);
						}
					}
					Text.NL();
				...
				Text.Add("You want [name], and you want [himher] now. Pulling the elf close, you whisper that you need to feel [hisher] [kCockDesc] inside you, that you want [himher] to take your [anus].", parse);
				Text.NL();
				if(kiakai.subDom.Get() < -30)
...
Scenes.Kiakai.AnalCatchPrep = function(choice, assert) {
	...
	var parse = {
		...
		belly  : function() { return player.StomachDesc(); },
		skin     : function() { return player.SkinDesc(); },
		tail     : function() { return player.HasTail().Short(); }
	};
	...
	if     (choice == Kiakai.AnalCatchPrepScene.Rim) {
		...
		Text.NL();
		Text.Add("The elf has plenty of oral experience it seems, as [heshe] expertly weakens your resistance, tongue probing deep inside you. Just as you are starting to get really hot and bothered, [name] withdraws from your lathered [anus], licking [hisher] lips. Time for the main course.", parse);
	}
		...
		Text.Add("You produce a vial of slick oil from your bags. The liquid is cool on your [skin] as the elf generously applies it to your [anus], working some of the sticky substance into your passage. [name] pours the rest on[oneof] [hisher] [kMultiCockDesc], spreading the soothing lubricant with long, slow strokes.", parse);
		Text.NL();
		Text.Add("The elf's [kCockDesc] is glistening, drops of excess oil splattering down on your waiting [butt]. You are as ready to have [himher] take you as you'll ever be.", parse);
	}
	
	Text.NL();
	parse["balls"] = player.HasBalls() ? Text.Parse(", pulling your [balls] aside", parse) : "";
	Text.Add("<i>“H-here I come, [playername]!”</i> [name] announces nervously. You lean back[balls], exposing your [anus]. A seductive wave of your hand is all it takes for the elf to accept your invitation, as [heshe] almost jumps on you in [hisher] eagerness. Guiding [hisher] shaft into position, [name] presses [hisher] [kCockTip] against your inviting anus. Thanks to the generous lubrication and a little effort, the elf easily penetrates you, pushing the first few inches of [hisher] member inside you.", parse);
	Text.NL();
	...
	if(player.FirstCock()) {
		...
		parse["itsTheir"] = player.NumCocks() > 1 ? "their" : "its";
		parse["s"]        = player.NumCocks() > 1 ? "s"     : "";
		Text.Add(" Your own [cocks] [isAre] straining to get free, trapped against [name]'s [kStomachDesc] as pre-cum leaks in generous amounts from [itsTheir] tip[s].", parse);
	}
	if(kiakai.HasBalls())
	...
	options.push({ nameStr : "Take charge",
		func : function() {
			...
			scenes.AddEnc(function() {
				...
				Text.NL();
				Text.Add("Not waiting for [hisher] response, you twist your [tail], pressing its tip against [name]'s [kAnusDesc]. The elf cries out as you penetrate past [hisher] meager defenses, burying your [tail] deep inside [himher]. Using your hands, you guide the prehensile appendage as far up [name]'s butt as it will go, filling [himher] completely.", parse);
				Text.NL();
			...
			if(tailpegFlag) {
				Text.Add("Not that you are by any means making that easy for [himher]. In addition to your [anus] constricting around [hisher] [kCockDesc], your tail is shoved so far up [hisher] [kAnusDesc] that it's unlikely the elf will be able to sit for quite some time. With each bounce, the appendage squirms inside [name]", parse);
			...
			if(Math.random() < expFraction) {
				...
				if(kiakai.FirstCock()) {
					...
					Text.Add(" The elf pours [weakStrong] jets of cum directly into your stuffed [anus], rapidly filling you as you milk [hisher] [kCockDesc].", parse);
				}
				else
				...
				if(jerkFlag) {
					...
					Text.Add("While the elf may have shot [hisher] load, that doesn't mean you are done with [himher] just yet. The [tail] stuck up [name]'s butt, rather than slowing down, rapidly pumps [hisher] innards", parse);
					if(kiakai.FirstCock())
				...
				Text.NL();
				if(suckFlag)
					Text.Add("A rather rhetorical question, as the elf couldn't answer you if [heshe] wanted to, being busy sucking on your [cock].", parse);
				else {
				...
				Text.NL();
				parse["balls"] = player.HasBalls() ? Text.Parse("a surge in your [balls]", parse) : "a tightening of your loins";
				Text.Add("Without regard for the moaning elf beneath you, you continue to ram your hips down, impaling yourself on [hisher] [kCockDesc]. Eventually, you can feel your own climax approaching, heralded by [balls]. Crying out triumphantly, you feel your [anus] clench around [name]'s still twitching [kCockDesc], each downthrust sending a jolt of pleasure up your spine.", parse);
				Text.NL();
				var cum = player.OrgasmCum();
				if(player.FirstCock()) {
					...
					if(suckFlag) {
						...
						Text.Add("True to your word, your let your [cocks] dump [itsTheir] [considerable] load all over your horny companion, coating [hisher] face, hair and [kBreastDesc] with sticky semen.", parse);
					}
				}
				else {
			...
			// You cum first
				...
				Text.Add("Despite your mocking bravado, you are the first one to cum, crying out as you feel your climax wreck your body. Judging by the way that [name]'s [kCockDesc] is twitching in the crushing confines of your [anus], [heshe] probably won't be far behind.", parse);
				var cum = player.OrgasmCum();
				if(player.FirstVag())
					Text.Add(" Your clear juices drip from your squirting [vag], splattering onto [name]'s [belly].", parse);
				if(player.FirstCock()) {
					if(suckFlag) {
				...
				if(kiakai.FirstCock()) {
					...
					if(cum > 3) {
						...
						Text.Add(" [name]'s [kBallsDesc] seems like [itThey] [hasHave] an unending amount of sperm, easily filling your back passage within seconds. More and more white goo pumps into your used rectum, slightly inflating your [belly].", parse);
					}
				}
				if(jerkFlag) {
				...
				if(tailpegFlag)
					Text.Add(" [name]'s [kAnusDesc] clenches tightly around your [tail], as if trying to milk it.", parse);
			}
			Text.Flush();
	...
	options.push({ nameStr : "More",
		func : function() {
			...
			if(player.FirstCock()) {
				...
				parse["s"]     = player.NumCocks() > 1 ? "s" : "";
				parse["notS"]  = player.NumCocks() > 1 ? "" : "s";
				Text.Add("Your [cocks] rub[notS] against [name]’s [kStomachDesc], trapped between you. [HeShe] reaches down, wrapping [hisher] slender fingers around[oneof] your member[s], jerking it in time with [hisher] hip movements.", parse);
				Text.NL();
			}
			else if(player.FirstVag() && kiakai.NumCocks() > 1) {
				parse["oneof"] = kiakai.NumCocks() > 2 ? " one of" : "";
				parse["s"]     = kiakai.NumCocks() > 2 ? "s" : "";
				Text.Add("As your bodies squirm together,[oneof] [name]’s unattended cock[s] brushes against your wet [vag], the undercarriage teasing your labia.", parse);
				Text.NL();
			}
			...
			scenes.AddEnc(function() {
				...
				Text.Add("You lie back and enjoy your companion's relentless pounding of your [anus], occasionally playing with yourself, or [br] [name]’s [kBreastDesc].", parse);
				Text.NL();
	...
	options.push({ nameStr : "Beg",
		func : function() {
			...
			if(cockFlag) {
				...
				Text.NL();
				Text.Add("With the means of your pleasure taken out of your hands, all you can do is lie back and enjoy [name]'s ministrations as [hisher] hand explores your needy cunt and [hisher] [kCockDesc] rapidly thrusts into your [anus].", parse);
			}
			else if(breastFlag) {
					...
					options.push({ nameStr : "Inside",
						func : function() {
							...
							if(cum > 3)
								...
								Text.Add(" The elf pulls out, the last strand of cum still connecting the [kCockTip] of [hisher] [kCockDesc] to your [anus].", parse);
							Text.NL();
							...
							Text.NL();
							Text.Add("[name] snuggles close to you, one hand busying itself with your soppy [anus] while the other plays with your [genDesc], urging you on.", parse);
