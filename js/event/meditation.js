...
Scenes.Meditation.Entry = function() {
	...
	scenes.AddEnc(function() {
		...
		if(lust >= 0.7 && kiakai.FirstBreastRow().Size() >= 7.5) {
			...
			Text.NL();
			Text.Add("In your current aroused state, you’re as helpless as a fly drawn to a bowl full of sugar. That magnificent elf-rack, each heavy hemisphere straining against [hisher] [toparmor], leaving just enough to the imagination. With each breath [Kiai] draws in, you see those sumptuous lady lumps rise and swell, [hisher] [toparmor] growing ever so slightly tighter. Then, on the downstroke, they shrink ever so slightly, making them seem ever so grabbable…", parse);
			Text.NL();
