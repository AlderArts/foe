...
Scenes.Miranda.HomeDommySexRideDobieCockVag = function() {
	var parse = {
		...
		hip    : function() { return player.HipDesc(); },
	...
	Text.NL();
	Text.Add("You hover there for a moment longer, letting the sweet promise of your hot cunny mesmerize the guardswoman. You have her in quite a nice position, but there’s a hungry glint in her eyes; a predator waiting to spring.", parse);
	Text.Flush();
	...
	options.push({ nameStr : "Take oral",
		func : function() {
			...
			var scenes = new EncounterTable();
			scenes.AddEnc(function() {
				Text.Add("Miranda quickly digs in, her tongue eagerly lapping at your [cocks]. Her eyes lock with yours as her hands roams, trying to judge your reactions as she explores your body. You look down confidently at the herm, satisfied at her compliance. It feels wonderful when she wraps her lips around[oneof] your shaft[s], alternately sucking and stroking you.", parse);
				Text.NL();
			...
			scenes.AddEnc(function() {
				...
				Text.NL();
				Text.Add("Her hands refuse to be idle, exploring your body and probing you relentlessly; now trailing a line down your side, barely touching your hip; now burying deep between your ass-cheeks, not-so-gently seeking out your hidden rosebud within.", parse);
				Text.NL();
			...
			Text.NL();
			Text.Add("She <i>has</i> earned it, you concede. Having just climaxed, you’re a bit unsteady, but with the help of Miranda’s guiding hands you’re soon back in your former position, straddling her rigid shaft, tip playing at your raw entrance. This time, the dobie doesn’t wait for your permission before pulling you down and thrusting her thick crimson pillar into your [vag].", parse);
			Text.NL();
...
Scenes.Miranda.HomeDommySexRideDobieCockVagSubmit = function(submit) {
	...
	Text.Add("Once she’s gotten started, there’s only one thing that’s gonna stop her: pouring a gallon of hot cum into your womb. Each time she drives into you, you can feel her heavy balls slap against your [skin]; a potent promise of future stickiness if ever there was one.", parse);
	Text.NL();
	...
	Text.Add("<i>“It goes against my instincts to fuck you like this,”</i> the herm grunts, leaning in closer for a kiss[b]. <i>“They tell me to turn you over[l] and fuck you senseless, breed you and seal the deal with my knot. I bet you’d love that, wouldn’t you?”</i> she grins. <i>“But… it’s also fun to see your face as I fuck you, [s].”</i>", parse);
	Text.NL();
...
Scenes.Miranda.HomeDommySexFuckDobieAss = function(cocks) {
	...
	if(dom < 0 && Math.random() < 0.5)
		Text.Add("You swat it away, ignoring her growling complaints as you apply a generous amount of saliva to your [cockDesc], preparing Miranda for penetration with your own natural lubricant. Her eyes show a hint of hesitation, but she grits her teeth and accepts it.", parse);
	else
...
Scenes.Miranda.HomeDommySexRideDobieCockAnal = function() {
	...
	Gui.Callstack.push(function() {
		...
		if(player.FirstCock()) {
			...
			if(load > 6)
				Text.Add("Gush after massive gush jets from your squirting [multiCockDesc], absolutely drenching the dog-morph's head and upper torso in your seed. Her face is liberally plastered in off-white spunk, her breasts similarly coated and a veritable river of semen drools down the canyon of her cleavage and over her belly.", parse);
			else if(load > 3)
		...
		options.push({ nameStr : "Take knot",
			func : function() {
				...
				Text.NL();
				Text.Add("It's an indescribable experience as the near-endless cascade of girl-jizz stretches you out fatter and fuller; by the time Miranda shudders her last, your stomach could pass for a pregnant woman's somewhere in her third trimester. It's so big and heavy it almost reaches down to rest atop Miranda's own stomach, and unconsciously you lean back to better support yourself with your newly increased girth.", parse);
				Text.NL();
...
Scenes.Miranda.HomeSubbySexTakeAnal = function(location, Loc) {
	...
	if(player.Slut() >= 50) {
		...
		Text.NL();
		Text.Add("<i>“Did you imagine this was what was going to happen when you followed me here?”</i> Miranda murmurs as she cups your cheeks, something wet and stiff poking you in the back. She rubs her breasts against your back, inexorably pushing you forward until you are bent over [loc1].", parse);
		Text.NL();
	...
	Gui.NextPrompt(function() {
		...
		if(player.FirstVag())
			Text.Add(" Your [vagDesc], feeling left out, aches for your attention. Almost without you thinking about it, your [hand] sneaks its way down there, fingering your wet folds.", parse);
		Text.NL();
					...
					options.push({ nameStr : "Reversal",
						func : function() {
							...
							if(p1Cock.isStrapon)
								...
								Text.Add("Sliding off Miranda’s softening cock, you make a great show of stroking your own [multiCockDesc], gazing down at her through eyes clouded with lust. If she doesn’t get what you’re planning to do, she’s sure to in just a moment.", parse);
							Text.NL();
...
Scenes.Miranda.TavernSexBackroomPrompt = function() {
	...
	if(miranda.Attitude() > Miranda.Attitude.Neutral) {
		if(dom > 25)
			Text.Add("<i>“Feel like using your favorite chewing toy, huh? Let’s go then,”</i> Miranda says, getting up and shaking her butt teasingly at you as she walks toward the backrooms.", parse);
		else
			Text.Add("<i>“Sounds like a nice idea, come on,”</i> she says, patting your back and pushing you toward the backrooms. Grinding against your back so you can feel her hardening shaft.", parse);
	}
	else
