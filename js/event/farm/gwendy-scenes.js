...
GwendyScenes.ChallengeSex = function(skillcheck, lose) {
	...
	if(lose || skillcheck < 20) {
		...
		Gui.NextPrompt(function() {
			var options = new Array();
			var ret = GwendyScenes.ChallengeSexLostPrompt(false, options);
			if(ret)
				Gui.SetButtonsFromList(options);
		});
	}
		...
		Text.Flush();
		var options = new Array();
		GwendyScenes.ChallengeSexWonPrompt(false, options);
		Gui.SetButtonsFromList(options);
	}
}
	...
	if(wins >= 5) {
		if(player.FirstCock()) {
			options.push({ nameStr : "Anal",
				func : function() {
					GwendyScenes.ChallengeSexAnal(false, hangout);
				}, enabled : true,
		...
		if(gwendy.flags["Toys"] == 0) {
			options.push({ nameStr : "Toys",
				func : function() {
					GwendyScenes.ChallengeSexAnal(true, hangout);
				}, enabled : true,
...
GwendyScenes.ChallengeSexAnal = function(toys, hangout) {
	...
	if(toys) {
		if(gwendy.flags["Toys"] == 0) {
			...
			GwendyScenes.ChallengeSexAnalToys(toys, hangout);
	}
	else // Domanal
	{
		if(first) {
