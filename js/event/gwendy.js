...
Scenes.Gwendy.LoftPrompt = function() {
	...
	Text.AddOutput("PLACEHOLDER: Loft.", parse);
	Text.Newline();
...
Scenes.Gwendy.BarnPrompt = function() {
	...
	// TODO: Initial talk
	Text.AddOutput("PLACEHOLDER: Barn.", parse);
	Text.Newline();
	Text.AddOutput("", parse);
	Text.Newline();
	Text.AddOutput("", parse);
...
Scenes.Gwendy.FieldsPrompt = function() {
	...
	Text.AddOutput("PLACEHOLDER: Fields.", parse);
	Text.Newline();
	Text.AddOutput("", parse);
	Text.Newline();
	Text.AddOutput("", parse);
...
Scenes.Gwendy.WorkMilking = function() {
	...
	options.push({ nameStr : "Lose",
		func : function() {
			lose = true;
			Text.Clear();
			Text.AddOutput("You accept her challenge, but in the back of your mind you decide to just lose for the sake of it. Given her demeanor and allure, a part of you wants to see what the losing side is like!", parse);
			PrintDefaultOptions();
		}, enabled : true,
	...
	Gui.Callstack.push(function() {
		...
		var scenes = new EncounterTable();
		scenes.AddEnc(function() {
			Text.AddOutput("You get ready to work, placing your tools beside a cow in one of the pens. The animal moos appreciatively as you ease her heavy burden.", parse);
		}, 1.0, function() { return true; });
		scenes.AddEnc(function() {
			Text.AddOutput("Your first customer of the day is a busty cowgirl, her breasts heavy with milk. She blushes brightly as you get into position, grabbing hold of her puffy nipples.", parse);
		}, 1.0, function() { return true; });
		scenes.AddEnc(function() {
			Text.AddOutput("You hear a moan from a nearby pen as you prepare your tools. Curiosity kindled, you head over and peek inside, blushing slightly at the scene before you. One of the cowgirls seems to have gotten started early, milk dripping from her immense knockers. She has her back propped up against the back of the stall, one hand massaging her freely lactating breasts and the other busy between her spread legs.", parse);
			Text.Newline();
			Text.AddOutput("At your arrival, she moans pleadingly, shuddering as you pull on her teats, letting the milk flow into one of your buckets.", parse);
			Text.Newline();
			Text.AddOutput("<i>\"Hey! No playing around with the livestock, you have work to do!\"</i> Gwendy shouts to you, already getting started on her second customer. You regretfully finish your task, leaving the girl to take care of her desires herself as you move on to the next one in line.", parse);
		}, 1.0, function() { return true; });
		scenes.Get();
		Text.Newline();
		...
		skillcheck -= gwendy.flags["WorkMilked"];
		...
		if(skillcheck < 20) {
			...
			Text.AddOutput("Given you rather quick reflexes and reactions, you manage to fill a good amount of buckets, even surpassing Gwendy from time to time. Still, it's hard and you make mistakes too often, letting Gwendy get ahead of you while you correct them.", parse);
		}
		else if(skillcheck < 60) {
			Text.AddOutput("While the work is hard, you carry on with deft skill, sometimes leaving Gwendy a bucket or two behind. However, it seems your attendees aren't the best you've had, as they often kick your precious cargo down. Oh well, you've the speed to make up for it, at least.", parse);
		}
		else { // +60
			Text.AddOutput("With your skill, it takes little to no time at all to fill your buckets, making Gwendy's efforts look meager. Time and again, you spot her looking at you as you pass by her to grab more buckets. Even when a stray kick threatens to spill one of your buckets, you quickly react, managing to save the milk. Sometimes, you even lend Gwendy a hand, just to keep her up to speed.", parse);
		}
		Text.Newline();
		...
		Text.AddOutput("It takes about [numhr] hours, but the two of you manage to milk those in need. Your hands feel a little raw, but it's nothing that'll stop you from performing your everyday tasks.", parse);
		Text.Newline();
		Text.AddOutput("On the up side, ", parse);
		
		// Boost stats
		var scenes = new EncounterTable();
		scenes.AddEnc(function() {
			Text.AddOutput("you could swear your hands work and move more deftly than before, which is always an improvement.", parse);
			player.dexterity.IncreaseStat(30, 1);
			return true;
		}, 1.0, function() { return player.dexterity.base < 30; });
		scenes.AddEnc(function() {
			Text.AddOutput("your time with milking the cow populace has left you slightly more educated about how to handle these situations, which could prove useful down the road.", parse);
			player.intelligence.IncreaseStat(30, 1);
			return true;
		}, 1.0, function() { return player.intelligence.base < 30; });
		scenes.AddEnc(function() {
			Text.AddOutput("you smile a little as you think back to kneading the teats of a particularly well-endowed cowgirl, and the numerous other breasts you fondled while working. Maybe this isn't so bad.", parse);
			player.libido.IncreaseStat(30, 1);
			return true;
		}, 1.0, function() { return player.libido.base < 30; });
		if(scenes.Get() == null) {
			Text.AddOutput("you had a good time, and the farmer really appreciated your help.", parse);
		}
		
		Text.Newline();
		Text.AddOutput("Gwendy stretches as she lets out a small yawn. <i>\"Well, I don't know about you but I still have a farm to handle. Thanks for the help, [playername], you saved me quite some time. So, help yourself to a bottle of milk, if you'd like.\"</i> You thank her, admitting you were a little parched. <i>\"It's fine, I have gallons of the stuff.\"</i>", parse);
		Text.Newline();
		
		// TODO: Gain milk item
		var scenes = new EncounterTable();
		scenes.AddEnc(function() {
			Text.AddOutput("She hands you a bottle of milk.", parse);
			party.inventory.AddItem(Items.CowMilk);
		}, 8.0, function() { return true; });
		scenes.AddEnc(function() {
			Text.AddOutput("She hands you a bottle of goat milk.", parse);
			party.inventory.AddItem(Items.GoatMilk);
		}, 8.0, function() { return true; });
		scenes.AddEnc(function() {
			Text.AddOutput("She hands you a bottle of sheep milk.", parse);
			party.inventory.AddItem(Items.SheepMilk);
		}, 8.0, function() { return true; });
		scenes.AddEnc(function() {
			Text.AddOutput("She hands you a bottle of Bovia.", parse);
		...
		*/
		...
		if(challenge) {
			...
			Text.AddOutput("With that, the two of you part, her heading to the barnyard, and you going your own way.", parse);
			if(party.Two()) {
				parse["himher"] = party.Get(1).himher();
				Text.AddOutput(" Your companion waits for you patiently at the entrance to the farm, and smiles as you join [himher] and set off.", parse);
			}
			else if(!party.Alone())
				Text.AddOutput(" Your companions are chatting among themselves while waiting for you at the entrance to the farm, smiling as you join them and set off.", parse);
			Gui.NextPrompt();
		}
	});
}
...
Scenes.Gwendy.ChallengeSex = function(skillcheck, lose) {
	...
	if(lose || skillcheck < 20) {
		...
		Text.AddOutput("<i>\"Seems like I'm the winner!\"</i> Gwendy concludes as she counts the tally, eager to get to the action.", parse);
		Text.Newline();
		
		if(gwendy.flags["LostChallenge"] < 3)
			Text.AddOutput("She won this time... meaning you've got to pay up to her in whatever way she demands. Your thoughts are confirmed when she looks at you with a sly smile.", parse);
		else if(gwendy.flags["LostChallenge"] < 6)
			Text.AddOutput("Despite your efforts, it seems like you can't beat the girl in her domain. A part of you wonders why you continue with these challenges if the result is always you losing and letting her degrade you. Still, you've got to try to best her somehow... after she reaps her reward from you, at least.", parse);
		else
		...
		Text.AddOutput("Counting the tally, it seems you came out the victor this time!", parse);
		Text.Newline();
		
		if(gwendy.flags["WonChallenge"] < 3)
			Text.AddOutput("It looks like you've bested the farm girl, and she pouts a bit. Still, a bet was a bet, and she's going to have to pay up!", parse);
		else if(gwendy.flags["WonChallenge"] < 6)
			Text.AddOutput("Seems like she's still going to challenge you, even though you've bested her so far! Her defiance has definitely increased her efforts whenever you challenge her, but the end result is the same: her waiting on your whim.", parse);
		else
			Text.AddOutput("At this point, it's hard to call it a challenge. Despite that, Gwendy has definitely given it her all to best you, it just that her best good enough. A shame, but it means you're going to have some fun...", parse);
		...
		func : function() {
			...
			if(gwendy.flags["ChallengeWinScene"] == 0) {
				if(gwendy.flags["LostChallenge"] > 0)
					...
					Text.AddOutput("Since this <i>was</i> the first time the two of you actually had the challenge, you decide to start things off light.", parse);
				Text.Newline();
				...
				Text.AddOutput("You decide to let her off easy this time, and only ask for a kiss.", parse);
			Text.Newline();
			Text.AddOutput("Grabbing Gwendy's arm, you pull her close, a surprised expression on her face as you hold her. For a good minute, you just take in her beauty, from the freckles on her cheeks to the lively blue in her eyes. You begin forming an idea or two on what to do next time, should you challenge her again and win.", parse);
			Text.Newline();
			Text.AddOutput("The farmer fidgets, a bit uncomfortable under your scrutiny. Well, you can't have that, so you get on with it. You kiss her, your lips flush with her soft ones. There's a slight gasp of shock at the sudden action, but it is soon succeeded by a more submissive moan as you press against her, your body leaning on her slightly.", parse);
			Text.Newline();
			
			if(hangout) {
				Text.AddOutput("You break the kiss before you get too hot and carried away, though you note the flush of arousal on her cheeks. With a taunting grin, you tell Gwendy that's all for now, teasing her a bit.", parse);
				Text.Newline();
				Text.AddOutput("<i>\"Getting me all worked up like that over nothing?\"</i> The farmer huffs, liking what she got but clearly expecting more.", parse);
			}
			else {
				Text.AddOutput("You break the kiss before you get too hot and carried away, though you note the flush of arousal on her cheeks. With a taunting grin, you tell Gwendy that's all for now, but the next time you challenge her, you won't let her off so easily. She winces slightly, but has a small smile on her face.", parse);
				Text.Newline();
				if(gwendy.flags["LostChallenge"] > 0)
					Text.AddOutput("<i>\"Don't think this one was anything but a fluke. I've beaten you before and I'll do it again!\"</i> Even though her face is flushed, she is wearing a determined look. Next time might not be so easy.", parse);
				else
					Text.AddOutput("<i>\"That's fine, [playername],\"</i> she retorts, <i>\"since this time was just a fluke. Don't count on winning so easily next time!\"</i> Good, you like it when the competition fights back. It makes winning just that much more fun.", parse);
			}
		...
		if(gwendy.FirstVag()) {
			if(player.FirstCock()) {
				options.push({ nameStr : "Fuck her",
					...
					tooltip : "The girl is begging for it, obligue her and ravage her cunt."
				});
			}
			if(player.FirstVag()) {
...
Scenes.Gwendy.ChallengeSexHands = function(cock, hangout) {
	var parse = {
		...
		gBreastDesc   : function() { return gwendy.FirstBreastRow().Short(); }
	};
	...
	if(first) {
		...
		if(player.Armor())
			Text.Add(" You remove your [armorDesc], smiling excitedly, imagining what's to come.", parse);
	...
	if(cock) {
		...
		Text.NL();
		Text.Add("Wrapping a hand around[oneof] your [multiCockDesc], Gwendy starts to jerk it with languid strokes. Her soft hands run from the top to the bottom of your length a few times, coaxing your [cockDesc] to full mast. A tingle of pleasure runs through you every time her fingers flit across your glans, a drop of pre forming on the [cockTip] in response to her gentle touch.", parse);
		if(first)
			Text.Add(" Feeling the sticky fluid being rubbed into her fingers and down your [cockDesc], she grimaces slightly, speeding up to get this over with.", parse);
		else
			Text.Add(" Feeling the sticky fluid being rubbed into her fingers and down your [cockDesc], she grins, increasing her speed while watching your face closely.", parse);
		Text.NL();
		Text.Add("The sensation of her fingers gliding smoothly along your length is overwhelming, and you buck in pleasure. You begin to lose yourself to the feeling each time she strokes the glans of your [cockDesc], trying to hold yourself back from release, but ", parse);
		if(player.HasBalls())
			Text.Add("your [ballsDesc] have something different in mind. They draw tight, the cum practically churning as if they are about to burst.", parse);
		else
			Text.Add("there is a familiar surge rising in your groin, despite your best efforts.", parse);
		Text.NL();
		Text.Add("Unable to take anymore, you cum hard, shooting your seed into the air and onto her face and her barely covered [gBreastDesc]. She gasps sharply, but she keeps her hands working until they've milked every ounce of your spunk, the sticky liquid running down her fingers.", parse);
		Text.NL();
		if(player.FirstVag()) {
			Text.Add("Your [vagDesc] joins in on the fun, leaking of girly juices as you ride your climax out. It's times like these that you enjoy being a fully functioning member of both genders.", parse);
			Text.NL();
		}
		...
		Text.NL();
		if(hangout) {
			Text.Add("With an appreciative moan, Gwendy licks her fingers clean. The entire time she looks into your eyes intently, looking forward to more. Shaking your head, you tease the lovely farm girl that this will be all, for now. She pouts sulkily, her sultry look replaced with mock anger.", parse);
			Text.NL();
			Text.Add("<i>\"I dunno how I feel about that... but I guess I'll let it slide for now.\"</i>", parse);
		}
		else
			Text.Add("Much to your surprise, Gwendy faces you with a sultry smile and begins to lick her fingers clean, making sure to draw her tongue around her lips and fingers in as seductive a way as possible. It looks like she still has a bit of fire left, but that's how you like it. Doing this without a fight would make it too easy to properly enjoy, though you keep that to yourself as you head off.", parse);
	}
	else {
		Text.Add("Smiling, you reveal your [vagDesc], as you're rather curious to see just how quick she is with her fingers. Spreading your legs wide, you tell her to get you off using her fingers.", parse);
		if(player.FirstCock()) {
			parse["balls"] = player.HasBalls() ? Text.Parse(" and your [ballsDesc]", parse) : "";
			Text.Add(" You pull your [multiCockDesc][balls] aside, revealing your eager slit.", parse);
		}
		if(!hangout)
		...
		Text.Add("The pace is slow but vastly enjoyable, as Gwendy gets [fingernum] fingers into you, teasing and rubbing against your [clitDesc] in the process. The entire time you find yourself grinding against her, almost threatening to take her knuckle-deep while you get wetter and wetter. Whenever she brushes or flicks your [clitDesc], you arch your back and moan like a slut, which gets a smile from her every now and then.", parse);
		Text.NL();
		...
		if(!hangout) {
			...
			Text.NL();
			Text.Add("After redressing, you kiss Gwendy on her forehead before heading off, but not without hinting about what you want to do the next time you beat her. She looks a bit defiant, but perhaps not as much as you expected.", parse);
		}
		else {
			...
			Text.NL();
			Text.Add("Nodding, you get up and redress before heading on your way.", parse);
		}
	}
...
Scenes.Gwendy.ChallengeSexBody = function(titjob, hangout) {
	...
	var parse = {
		...
		gBreastDesc    : function() { return gwendy.FirstBreastRow().Short(); },
		gMultiCockDesc : function() { return gwendy.MultiCockDesc(); },
		gButtDesc      : function() { return gwendy.Butt().Short(); }
	};
	...
	if(first) {
		...
		Text.Add("Once more, you lead her to her room for some privacy after your win. Grabbing a chair from the table and directing Gwendy to kneel before it, as you undress and sit down.", parse);
	}
	else {
		Text.Add("Quickly removing your [armorDesc], Gwendy kneels before you, looking up expectantly.", parse);
		Text.NL();
	...
	if(titjob) {
		...
		parse["itsTheir"] = player.NumCocks() > 1 ? "their" : "its";
		Text.Add("Your [multiCockDesc] poke[notS] out, swelling eagerly as you caress [itThem]. You tell Gwendy to use her breasts today, seeing as she's always putting them on display.", parse);
		if(!hangout)
		...
		Text.NL();
		Text.Add("Propped up by her hands, her breasts slide around your [multiCockDesc] as she glides up and down [itsTheir] length[s] - without friction, thanks to your fluids. You are almost ready to go blow from the feeling alone, but you try to restrain yourself and last as long as you can.", parse);
		Text.NL();
		if(player.HasBalls())
			Text.Add("However, it seems your [ballsDesc] think otherwise, as they draw tight, ready to relinquish their load.", parse);
		else
		...
		if(!hangout) {
			...
			Text.NL();
			Text.Add("With that thought, you redress and bid the girl farewell, thinking of what you're going to do next.", parse);
		}
		else {
			...
			Text.NL();
			Text.Add("You mull over that as you redress, minding the erect reminder she left you.", parse);
		}
		...
		Text.Add("Despite having stripped down, you tell her she's to do nothing to you. She looks at you with a surprised look, but you keep a straight face and tell her to strip herself. She complies and takes her clothes off in front of you, getting you excited. Her [gBreastDesc] spring free from their tight confines, bouncing slightly as gravity kicks in. Her form is slick, as a thin layer of sweat has built up over the course of today's work.", parse);
		if(gwendy.FirstCock())
		...
		Text.Add("Approaching Gwendy with a wistful look, you sit in her lap and kiss her deeply. You stay there for almost a minute, wrapping your tongue around hers, the both of you moaning softly. Before you lose control entirely, you break the kiss, both of you eager for more of each other. Remembering your goal, you resist the urge to indulge as you begin kissing her cheek, her neck, causing her to whimper cutely.", parse);
		Text.NL();
		if(first) {
		...
		Text.Add("Moving further down, you cup her plentiful breasts, hefting them in your hands as you give her nipples playful licks with the tip of your [tongueDesc]. You continue to tease and caress her with both your hands and your mouth, arousing both yourself and her, carefully avoiding her genitalia.", parse);
		Text.NL();
		if(gwendy.FirstCock()) {
			Text.Add("Despite your meticulous care, Gwendy is already rock hard. Still, with how things are going, you decide to give her her male endowment[s] some attention. With deft movements, you manage to trap her equipment between your thighs. Gwendy jerks at bit while you squirm seductively, teasing her trapped pecker[s] mercilessly.", parse);
			Text.NL();
		}
		...
		if(gwendy.Gender() == Gender.female)
			...
			Text.Add("impossibly rigid [gMultiCockDesc].", parse);
		else
		...
		Text.NL();
		parse["cock"] = gwendy.FirstCock() ? Text.Parse(" and stiff [gMultiCockDesc]", parse) : "";
		parse["gen"]  = gwendy.FirstVag()  ? Text.Parse("dripping cunny[cock]", parse) : Text.Parse("stiff [gMultiCockDesc]", parse);
		...
		if(player.FirstCock()) {
			...
			Text.Add("Your [multiCockDesc] twitch[notEs], ready to plunge into her depths. This really might get out of hand if you let it go on for too long...", parse);
			Text.NL();
		}
		Text.Add("Sauntering up to her and caressing her exposed back, your fingers trace down her spine to the crevice of her [gButtDesc], your fingers trailing down its middle. She lets out a small moan and pushes back against you like a slut, desperate for your touch. You are happy to sate this small request of hers. ", parse);
		
		if(gwendy.FirstCock() && gwendy.FirstVag()) {
			if(gwendy.HasBalls())
				Text.Add("Lifting up her ample balls gently, your index and middle fingers slide into the dripping box beneath it. Not to exclude her male additions, you roll and massage her balls in your palm as you frig her.", parse);
			else
...
Scenes.Gwendy.ChallengeSexOral = function(blow, hangout) {
	...
	if(blow) {
		...
		Text.Add("Confidently, you tell Gwendy to suck your [cockDesc].", parse);
		Text.NL();
		Text.Add("She sets about her duty without a word of complaint, grabbing[oneof] your cock[s] and giving it a few strokes, before experimentally licking the tip. You sigh with pleasure as she begins to kiss and lick you, all with a dismissive cute look in her eyes.", parse);
		Text.NL();
		...
		Text.NL();
		if(player.HasBalls()) {
			Text.Add("She massages your [ballsDesc] and kisses them from time to time, alternating between your dick and your balls in an effort to arouse you as much as possible.", parse);
			Text.NL();
		}
		if(player.NumCocks() > 1) {
			parse["s"]        = player.NumCocks() > 2 ? "s" : "";
			parse["itThem"]   = player.NumCocks() > 2 ? "them" : "it";
			Text.Add("While Gwendy focuses most of her attention on your main [cockDesc], she occasionally strokes your other dick[s], teasing [itThem] to full mast.", parse);
			Text.NL();
		}
		if(player.FirstVag()) {
			Text.Add("As well as pleasing your [cockDesc], she occasionally licks and laps away at your [vagDesc], causing you to moan. However, it's clear that your [cockDesc] holds the bulk of her interest for now.", parse);
			Text.NL();
		}
		...
		Text.NL();
		Text.Add("However, your resistance comes to an end when she slides a single finger into your [anusDesc] and presses against your prostate. The sensation is far too much for you to handle, as she probes and squeezes persistently, massaging your delicate tunnel.", parse);
		Text.NL();
		if(player.HasBalls())
			Text.Add("Your [ballsDesc] draw tight, the cum desperately trying to erupt from you as it flows up and out.", parse);
		else
		...
		Text.Add("Flashing your [vagDesc] in her face, you tell her to eat you out. You could use a good licking, and you're certain Gwendy won't object too much.", parse);
		if(!hangout)
		...
		Text.NL();
		Text.Add("The farmer is squirming, one of her hands getting busy in her own cleft, intent on getting some fun out of this too. She moans, and moves her tongue with stronger but slower laps, making you writhe in ecstasy, desperately trying to force your [vagDesc] further into her face. Your orgasm is closing in, and any attempts to stave it off are rendered futile by Gwendy's skillful mouth. Your muscles start spasming, and your [vagDesc] spurts its juices all over your lover's face.", parse);
		Text.NL();
		if(player.FirstCock()) {
		...
		if(hangout) {
			...
			Text.NL();
			Text.Add("<i>\"Quite the honeypot you have there, [playername]. Wouldn't mind tasting a bit more of you, if you get my drift.\"</i> You're happy she enjoyed it so much, but you have other matters to tend to. After giving your lover a kiss, you redress and head out.", parse);
		}
		else
			Text.Add("Exhausted, you see her drenched in fluids, though she's licking her lips as if they were coated in the most delicious liquid she's ever tasted. You redress and tell her you'll be back, if she still feels the need to challenge you. Heck, you'll probably be back anyways.", parse);
	}
...
Scenes.Gwendy.ChallengeSexVag = function(fuck, hangout) {
	...
	var parse = {
		...
		gBreastDesc    : function() { return gwendy.FirstBreastRow().Short(); },
		gMultiCockDesc : function() { return gwendy.MultiCockDesc(); },
		gVagDesc       : function() { return gwendy.FirstVag().Short(); },
		gBellyDesc     : function() { return gwendy.StomachDesc(); },
		gButtDesc      : function() { return gwendy.Butt().Short(); }
	};
	...
	if(first) {
		...
		Text.NL();
		Text.Add("Unlike your previous encounters, you remain standing after removing your gear, eying her hungrily. You see a little trepidation on Gwendy’s face for a change, but you don’t really care. Pushing Gwendy against the table, you ready yourself to ravage her [gVagDesc]!", parse);
		...
		Text.Add("With yet another win under your belt, you head back to the loft with the defeated farmer in tow. Once the two of you are secluded, you hastily strip as lust and greed begin to overtake you. Uncaring of the odd glances she casts at you, you push her onto the table, ready to claim her [gVagDesc] again!", parse);
	}
	else {
		Text.Add("With a lecherous smile, you gently push Gwendy against the table. Stripping feverishly, like a [manWoman] possessed, you kiss her greedily, humping against her. She moans appreciatively, her [gVagDesc] growing moist with arousal.", parse);
		Text.NL();
	...
	if(fuck) {
		...
		Text.NL();
		Text.Add("Moving faster and faster, your [cockDesc] is becoming almost painfully erect at this point, and your only options to sooth it is to bury it deep within the pretty girl underneath you. Gwendy, so consumed by pleasure that she is reduced to moaning and panting, wraps her legs around you while gripping the edges of the table tightly as you slam into her. Her eyes are half lidded, clouded by her lust, but her wordless cries beg you to fuck her harder, her [gVagDesc] clamping down fervently around you, as if to milk you dry.", parse);
		Text.NL();
		Text.Add("As much as you’d love to prolong this glorious fuck, you are approaching your limit, ", parse);
		if(player.HasBalls())
			Text.Add("your [ballsDesc] drawing tight, ready to pack her full of spunk like the bitch she is.", parse);
		else
		...
		if(hangout) {
			Text.Add("As you regain your composure, Gwendy pants breathlessly. With a chuckle, you consider the fuck well received before standing up and redressing. As you ready to leave, you look back one last time to see Gwendy blowing a flirtatious kiss.", parse);
			Text.NL();
	...
	// trib
		...
		if(player.FirstCock()) {
			Text.Add(" As you top her, your [multiCockDesc] [isAre] treated to a warm massage by the tight sleeve between your bodies. Just being pressed against her makes you want to cum, to spill your seed on both of you.", parse);
			if(gwendy.FirstCock())
		...
		Text.NL();
		Text.Add("Unable to deny the pleasure wrought from rutting against your slutty lover, you groan as you hit your climax, smearing the fluids that pour from your [vagDesc] onto the two of you. Your fluids mingle with those of Gwendy, the ecstatic look in her eyes telling you she is riding the crest of her own orgasm.", parse);
		if(player.FirstCock()) {
			Text.Add(" Your [multiCockDesc] join[notS] in on the fun, spraying an even coat of white glaze across Gwendy’s [gBellyDesc] and lower breasts.", parse);
			if(gwendy.FirstCock())
				Text.Add(" Just as you come, Gwendy adds her own jism to the fray, covering both of you with another layer of white fluid as she empties her equine balls.", parse);
		}
		else if(gwendy.FirstCock()) {
		...
		if(hangout) {
			...
			Text.NL();
			Text.Add("Before any heavy banter is started, you untangle yourself from your lover’s grip and redress. Once you are fully equipped and ready to go, Gwendy swats your [buttDesc] playfully. The farmer kisses you briefly again before sending you on your way.", parse);
		}
		else {
			Text.Add("You’d love to just stay there and cuddle with Gwendy forever, but unfortunately, you have other things to do. Cleaning yourself off, you redress and head on, leaving Gwendy to clean up after you, hoping she’ll challenge you again so you can continue the fun. As you leave, you swear you spot a burning determination in Gwendy’s eyes. Looks like the fight isn’t quite over yet...", parse);
		}
	}
...
Scenes.Gwendy.ChallengeSexAnal = function(toys, hangout) {
	...
	var parse = {
		...
		gBellyDesc     : function() { return gwendy.StomachDesc(); },
		gButtDesc      : function() { return gwendy.Butt().Short(); },
		gAnusDesc      : function() { return gwendy.Butt().AnalShort(); }
	};
	...
	// If first time
	if(first) {
		Text.Add("Yet again you’ve bested Gwendy at her own game, proving your worth to the curvaceous farmgirl. While she put up a good fight, it was simply not enough compared to your skills. You flash Gwendy a victorious smile, and she responds with a nervous grin, hanging her head a bit. While she is grateful for your help, she’s a bit fearful for what you might have planned this time.", parse);
		Text.NL();
		Text.Add("It seems most of the sultry farmgirl’s determination has left her body, but you’ve yet to reap your reward. Once you’ve reached the loft, you instruct for Gwendy to sit on her bed as you remove your [armorDesc]. Rather than throw yourself at her like she expected - perhaps wished for? - you simply pet her on the head, building the tension while eyeing her with a predatory smile.", parse);
		...
		Text.NL();
		Text.Add("The latter actually seems likely, as she helps you remove your [armorDesc] before hopping onto her bed, breathing heavily in anticipation.", parse);
	}
	else {
		Text.Add("Feeling that you want to have some fun, you constantly tease and taunt Gwendy about her rear, saying how fun it is to grope it, and how you’d like to stuff it, fill her to the brim.", parse);
		Text.NL();
	...
	{
		if(first) {
			Text.Add("Her shapely ass been teasing you for the longest time now, but the reward has been worth the wait. Gwendy is so far oblivious of your intentions, so you bring her up to speed by telling her to turn around and prop her ass up. She pouts slightly, but does as commanded, turning around and presenting her rear for you. She even goes the extra step and shakes her ass enticingly, looking back for your approval.", parse);
			Text.NL();
			...
			if(gwendy.FirstCock())
				Text.Add(" Her equine member bobs erratically, as if anticipating the impending ass fucking.", parse);
			Text.Add(" A low moan escapes the slutty girl as you rub[oneof] your [multiCockDesc] between her cheeks, quickly reaching full-mast.", parse);
		}
		else {
			Text.Add("Her shapely ass been teasing you for the longest, and today, you’re gonna ravage it yet again. Rubbing your [hand]s together, you tell Gwendy to get on all fours. She does so slowly, but once her butt is finally turned to you, she’s greeted with an abrupt spank, causing her to gasp sharply. You smile as you continue, pushing the girl’s face down onto her bed while your free hand kneads her buttocks.", parse);
			Text.NL();
			...
			if(gwendy.FirstCock())
				Text.Add("Even her flared cock seems raring for the reaming, twitching helplessly despite receiving no attention. ", parse);
			Text.Add("Stroking[oneof] your [multiCockDesc], you grind and hump the farmgirl’s supple cheeks, making her moan appreciatively as you continue.", parse);
			Text.NL();
		...
		if(Math.random() < 0.5)
			...
			Text.Add("<i>”T-top drawer!”</i> the farmer gasps as she feels your [cockTip] prodding her entrance. Delaying your conquest slightly, you pull out an unmarked bottle from said drawer, pouring a generous coating of the lubricant on both your [multiCockDesc] and your fingers. Starting out light, you sink two of your slick digits into her rear. ", parse);
		Text.NL();
		Text.Add("She arches her back, letting out a low whimper while you slowly work the fingers in and out of her [gAnusDesc] in an effort to get her prepared. All the while, you tease that she enjoys being finger-fucked like some common whore, which makes her blush shamefully.", parse);
		if(gwendy.FirstCock())
		...
		if(!hangout) {
			Text.Add("You are not going to hold back any more, as you simply want to drive into her that if she challenges you, she will be yours to use. It seems like she has arrived at the same conclusion, as she reaches back and spreads her own cheeks while bending as low as possible, trying to get the best angle for your [cockDesc]. Seems this girl has <i>a lot</i> more experience than you gave her credit for. Wonder who else has explored these depths?", parse);
			Text.NL();
			Text.Add("<i>”Good girl,”</i> you grunt, pleased with her submissive demeanor.", parse);
			Text.NL();
		}
		Text.Add("Thrusting and pumping into her savagely, you are rewarded with whorish moans and gasps. She hardly seems to mind. Thanks to you, she has been reduced to the most base of pleasures, begging for more as you repeatedly bury your [cockDesc] in her shapely butt. After your initial warm-up, you give in to your desires, fucking her with abandon, pushing both of you toward your inevitable climaxes.", parse);
		Text.NL();
		if(first)
	...
	if(toy == Gwendy.Toys.Strapon) {
		...
		options.push({ nameStr : "Work",
			func : function() {
				...
				if(gwendy.FirstCock())
					Text.Add(" Her [gMultiCockDesc] straining against the fabric of her tight shorts could clue someone in, though.", parse);
				Text.NL();
				...
				var scenes = new EncounterTable();
				scenes.AddEnc(function() {
					Text.Add("Gwendy doesn’t seem to want to leave the barn, so she goes to tend the cows. She doesn’t do a very good job at it though, as she winches visibly each time she sits down to milk one of them. She keeps throwing smoldering glances in your direction.", parse);
					if(!hangout)
				...
				scenes.AddEnc(function() {
					...
					Text.NL();
					if(gwendy.FirstCock()) {
						Text.Add("<i>”Gwendy, can you-”</i> [Adrian] falters as the farmer whirls about, faced with her barely contained, rock-hard [multiCockDesc]. <i>”I- uh, I’ll just be over here,”</i> [heshe] mumbles, quickly putting some distance between [himher]self and the horny, irritated farmer. She goes back to hacking angrily at the innocent grass, throwing glowering looks in your direction.", parse);
					}
					else {
				...
				Gui.NextPrompt(function() {
					...
					Text.NL();
					parse["gen"] = gwendy.FirstCock() ? Text.Parse(", freeing her stiff [gMultiCockDesc]", parse) : "";
		...
		Text.Add("Wanting to return the favor, Gwendy licks on the other end of the toy, coating the flare in her saliva. She reaches farther and farther, until you suddenly feel her cool tongue on your [buttDesc]. She lightly caresses it, hesitantly at first, but growing more and more confident as she buries her head between your cheeks, loosening up your pucker.", parse);
		if(player.FirstCock())
			Text.Add(" You almost regret your plan, why not throw her on her back and ram your [multiCockDesc] inside her?", parse);
		Text.NL();
		Text.Add("<i>”M-me first,”</i> Gwendy pants, getting up on all fours, shaking her ass at you invitingly. <i>”Use a lot of lube, okay?”</i> She sighs in pleasure as the cool lubricant pours between her cheeks, dripping down the huge toy as you slide it between her buns. Adjusting your aim slightly, you press the flared head against her back entrance. At first, it looks like an impossible task to fit the broad head equine toy inside her, but the girl keeps egging you on, spreading her legs and cheeks wide in her efforts to impale herself on her favorite dildo.", parse);
		Text.NL();
		...
		Text.NL();
		Text.Add("On second thought, you aren’t so sure about this anymore, but you let yourself be pulled down on top of the girl, closing your eyes as her lubed fingers snake their way into your tight anus, spreading it in preparation for the huge toy. After sharing a deep kiss, you nervously reposition yourself on your back, legs spread for the monster you will share with your lover.", parse);
		Text.NL();
		...
		Text.NL();
		if(player.FirstCock()) {
			Text.Add("Just like she said, having the studded horsecock plowing your behind for an extended period of time is bliss for your poor prostate. Already at the first thrust you could feel your cockmilk being pressed out of it, and by now your [multiCockDesc] [isAre] drooling with cum from numerous anal orgasms.", parse);
			Text.NL();
		}
	...
	if(lossScene >= 1) {
		options.push({ nameStr : "Make-out",
			func : function() {
				...
				if(player.FirstVag()) {
					Text.Add("As the two of you make out, you feel moisture trickles from your [vagDesc], lightly soaking your undergarments. Seeing your arousal on your face, Gwendy smirks a bit as she fingers you playfully. It feels good, oh so good, but Gwendy’s skilled fingers keep you teetering on the edge of orgasm, preventing you from reaching your climax.", parse);
					Text.NL();
				}
		...
		options.push({ nameStr : "Denial",
			func : function() {
				...
				if(!hangout) {
					...
					Text.Add("Gwendy smiles happily as she pushes you into a chair, her predatory expression rising submissive arousal through you. She starts off gently, kissing lovingly and passionately. As you two kiss, Gwendy’s fingers run down your hips and thighs.", parse);
				}
				Text.NL();
				...
				scenes.AddEnc(function() {
					...
					Text.NL();
					Text.Add("<i>“Good. Well, you’re free to leave now, since I’ve had my fun. Just make sure that if you do get off, it’d better be away from the farm. Who knows what might happen to you if I catch you?”</i> Despite her playful tone, you instinctively know you’d be degraded even worse than today. Redressing in haste, you mumble your goodbyes to Gwendy, before heading on your way, painfully aroused.", parse);
					Text.NL();
				}, 1.0, function() { return player.FirstCock(); });
				scenes.AddEnc(function() {
					Text.Add("Gwendy chuckles in a haughty tone, as she begins to caress the slick folds surrounding your [vagDesc]. It elicits a low moan from you and she smiles at your arousal. Slowly, she begins tease your [clitDesc] with a stray finger as you squirm and grow wetter.", parse);
					Text.NL();
					Text.Add("It feels heavenly, and you can hardly believe that she’s going to let you get off this easily. As her fingers dig into the cleft of your [vagDesc], however, your road to orgasm is stopped when you feel a sharp pain against your [clitDesc]. Looking down, you see Gwendy pinching it between her finger, making you whimper pathetically.", parse);
					Text.NL();
					if(!hangout) {
					...
					Text.Add("In addition to teasing your [clitDesc], she dips two fingers into your dripping [vagDesc], pumping and grinding the digits against your vaginal walls. All the time, you whimper and pant, ever close the edge of relief but always blocked off by Gwendy’s teasing.", parse);
					Text.NL();
					Text.Add("You hold on as best you can, but the teasing and denial mercifully end when Gwendy relaxes the pressure on your [clitDesc] and eases the intensity of her frigging. You almost come then and there, but a look at Gwendy tells you better.", parse);
					Text.NL();
				...
				Text.Add("The voice is playful, but does little to mask the obvious threat she’s laid out. As best you can, you redress and leave in haste, not wanting to risk incurring her sexual wrath on you.", parse);
...
world.loc.Farm.Loft.events.push(new Link(
	...
	function() {
		if(gwendy.IsAtLocation(world.loc.Farm.Loft)) {
			Text.AddOutput("Gwendy is here.");
		}
		else
			Text.AddOutput("Gwendy doesn't seem to be in at the moment.");			
		Text.Newline();
	},
