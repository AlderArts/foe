...
Scenes.Golem.OnLoss = function() {
	...
	if(player.FirstVag()) {
		...
		parse["tightLoose"] = player.Butt().stretch.Get() > 2 ? "loose" : "tight";
	...
	Gui.NextPrompt(function() {
		Text.Clear();
		parse["cock"] = player.FirstCock() ? ", covered in strands of your own seed" : "";
		Text.Add("As time passes, you come to again. You are a complete mess[cock], and tired beyond measure. At least a few hours have passed, even if your conception of time is still a bit fuzzy. You don’t see any trace of the golem, and the tower wall is once again unbroken, so you assume the guardian has returned to her original state.", parse);
		Text.NL();
