...
import { GAME, MoveToLocation, StepToHour, TimeStep, WORLD } from "../GAME";
...
export namespace CassidySexScenes {
	...
	export function Indoors() {
		...
		const first = !(cassidy.flags.Talk & CassidyFlags.Talk.SexIndoor);
	...
	export function IndoorPrompt(opts: any = {}) {
		...
		if (player.BiggestCock(undefined, true)) {
			options.push({nameStr : "Fuck her",
				...
				func() {
					CassidySexScenes.FuckHer(cocksInVag);
				},
			});
			options.push({nameStr : "Anal",
				...
				func() {
					CassidySexScenes.Anal(cocksInAss);
				},
			});
		}
	...
	export function Savor(opts: any) {
		...
		CassidySexScenes.IndoorPrompt(opts);
	}
...
export namespace CassidySparSexScenes {
	...
	export function Win() {
		...
		if (cassidy.flags.Talk & CassidyFlags.Talk.SexIndoor) {
				...
				func() {
					...
					options.push({nameStr : "Outside",
						...
						func() {
							...
							CassidySparSexScenes.WinPrompt();
						},
					});
	...
	export function Loss() {
		...
		if ((cassidy.flags.Talk & CassidyFlags.Talk.SexIndoor) && (Math.random() < 0.5)) {
	...
	export function DomRide() {
		...
		CassidySparSexScenes.LossOutro();
	}
