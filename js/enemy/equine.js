...
Scenes.Equine.WinFuckHer = function(enc) {
	...
	Text.Add("between her legs, not stopping until your [face] is suspended over hers and you are staring down into her wide, somewhat apprehensive eyes. Your [cocks] twitch[notEs] over her mons, and you enjoy the radiant heat that pulses between your respective genitals as you watch her expression grow more lustful and needy with each passing moment. Her eyes repeatedly dart down toward your crotch and then back up to meet your gaze, not understanding the lack of penetration or your reason behind it.", parse);
	Text.NL();
...
Scenes.Equine.WinRideHimVag = function(enc) {
	...
	Text.NL();
	Text.Add("After a few more twitches and spasms, the mare slumps to the side in a daze, and the stallion’s cock exits her gullet with loud slurp. Fortunately, he has not quite reached his peak, so his pillar of equine cockmeat stands tall and throbbing, glistening with the mare's saliva and a bit of early precum. If your [vag] ", parse);
	if(player.FirstCock())
	...
	if(player.FirstCock()) {
		if(cum > 6) {
			...
			Text.Add("Your [cocks] pulse[notS] with the onset of orgasm, but your contracting scrotum is simply too dried out to produce much cum. You manage a couple of pathetic squirts that barely reach the equine’s heaving pecs, followed by a thin strand that fails to gain enough momentum to detach from the head of[oneof] your tip[s], instead dripping down to briefly connect your cock to his pelvis. As if in apology for the weak display, your [vag] continues pleasurably spasming around his shaft for another solid minute, spurred on by the sporadic twitching of the equine’s bound cock that remains lodged firmly inside you. ", parse);
		}
		Text.NL();
	}
