...
OrchidScenes.Interact = function() {
	...
	var first = !(orchid.flags["Talk"] & OrchidBoss.Talk.First);
		...
		options.push({ nameStr : "Spring",
			...
			func : function() {
				...
				if(party.InParty(layla)) {
					...
					Text.NL();
					if(layla.flags["Talk"] & Layla.Talk.Origin) {
						...
						layla.flags["Talk"] |= Layla.Talk.Origin;
		...
		options.push({ nameStr : "Lizard?",
			...
			func : function() {
				...
				if(player.RaceCompare(Race.Reptile) >= 0.4) {
					...
					if(oasis.flags["Visit"] >= Oasis.Visit.Visited) {
