...
let EntityCombat = {
	...
	//TODO Structure
	FinishCastInternal : function(ability, encounter, caster, targets) {
		Text.Flush();
	...
	CanBeInterrupted : function(ability, encounter, caster, result) {
		return true;
	},


	GetCombatEntry : function(encounter) {
		var ent = this;
		var found;
	...
	GetPartyTarget : function(encounter, activeChar, ally) {
	...
	GetSingleTarget : function(encounter, activeChar, strategy, ally) {
		var isEnemy = activeChar.isEnemy;
		var confuse = activeChar.entity.combatStatus.stats[StatusEffect.Confuse];
		if(confuse)
			isEnemy = !isEnemy;
	...
	AddHPFraction : function(fraction) {
		fraction = fraction || 0;
		var old = this.curHp;
	...
	AddSPFraction : function(fraction) {
		fraction = fraction || 0;
		var old = this.curSp;
	...
	AddLustFraction : function(fraction) { // 0..1
		fraction = fraction || 0;
	...
	PhysDmgHP : function(encounter, caster, val) {
		var parse = {
	...
	AddLustOverTime : function(hours) {
		// TODO: Function
	...
	LustCombatEfficiencyLevel : function() {
	...
	LustCombatTurnLossChance : function() {
	...
	AddResistanceWear : function(type, wear) {
		wear = wear || 0;
