...
import { GAME, GameCache, TimeStep, WORLD, WorldTime } from "../../engine/GAME";
import { EntityStorage, MoveToLocation } from "../../engine/GAME";
import { Inventory } from "../../engine/inventory/inventory";
import { HalloweenItems } from "../../engine/inventory/items/halloween";
...
export class Halloween {
	...
	public static IsSeason() {
		// Always allow debug
		if (GetDEBUG()) { return true; }

		// Halloween season is 14 oct - 14 nov
		const date  = new Date(); // Get current datetime
	...
	public harthon: number;
	public harthonPreg: number;
...
// GameCache().flags["HW"]
const HWState = { // Bitmask for globally tracked flag
	Intro : 1,
	Pie   : 2,
};
...
export namespace HalloweenScenes {
	...
	export function PumpkinPie() {
		...
		const first = !(GameCache().flags.HW & Halloween.State.Pie);
	...
	HWLoc.Camp.events.push(new Link(
		...
		() => {
			...
			const first = !(HW.flags & HalloweenFlags.Flags.Elder);
			HW.flags |= HalloweenFlags.Flags.Elder;
	...
	export function Ronnie() {
		...
		const first = HW.ronnie === HalloweenFlags.Ronnie.NotMet;

		if (first) {
			HalloweenScenes.RonnieFirst();
		} else if (HW.ronnie === HalloweenFlags.Ronnie.PCBeta) {
	...
	export function RonnieFirst() {
		...
		let beenAround = false;
		if (HW.flags & HalloweenFlags.Flags.Graveyard) { beenAround = true; }
		if (HW.flags & HalloweenFlags.Flags.WitchHut) { beenAround = true; }
				...
				Gui.NextPrompt(() => {
					...
					options.push({ nameStr : "Fight!",
						...
						func() {
							...
							const hasCock = player.FirstCock();
							...
							options.push({ nameStr : "Leave",
								...
								func() {
									...
									HW.ronnie = HalloweenFlags.Ronnie.Removed;
	...
	export function WerewolfTF() {
		...
		// TODO
		const blessed = false; // HW.flags & HalloweenFlags.Flags.Nadirmasomething...
		...
		if (cocks.length === 0) {
			const cock = new Cock(Race.Wolf, Color.red);
			cocks.push(cock);
		}
		_.each(cocks, (cock) => {
		...
		// Set flags
		HW.flags |= HalloweenFlags.Flags.Werewolf;
	}
	...
	export function RonniePitch() {
		...
		const first = HW.ronnie !== HalloweenFlags.Ronnie.PCAlpha;
		HW.ronnie = HalloweenFlags.Ronnie.PCAlpha;
	...
	export function RonnieCatch() {
		...
		HW.ronnie = HalloweenFlags.Ronnie.PCBeta;
	...
	export function Kiai() {
		...
		const first = !(HW.flags & HalloweenFlags.Flags.Kiai);
		HW.flags |= HalloweenFlags.Flags.Kiai;
	...
	HWLoc.Mausoleum.description = () => {
		...
		if (first) {
			...
			HW.flags |= HalloweenFlags.Flags.Mausoleum;
		} else {
		...
		if (HW.harthon & HalloweenFlags.Harthon.Met) {
	...
	export function NadirMa() {
		...
		HW.flags |= HalloweenFlags.Flags.NadirMa;
		...
		const prompt1 = () => {
			...
			const prompt2 = () => {
				...
				if (!player.FirstCock()) {
					...
					const prompt3 = () => {
						HW.nadirma |= HalloweenFlags.NadirMa.GaveCock;
	...
	export function NadirMaCont(parse: IParse) {
		...
		const prompt = () => {
			...
			if (player.HasBalls()) {
				...
				Text.NL();
				if (HW.nadirma & HalloweenFlags.NadirMa.GaveCock) {
				...
				const prompt2 = () => {
					HW.nadirma |= HalloweenFlags.NadirMa.GaveBalls;
	...
	export function NadirMaCont2(parse: IParse) {
		...
		Text.NL();
		parse.b = (HW.nadirma & HalloweenFlags.NadirMa.GaveBalls) ? "" : "ly tweaked";
	...
	export function NadirMaNoEntry(parse: IParse, func: CallableFunction) {
		...
		const first = !(HW.nadirma & HalloweenFlags.NadirMa.SaidNo);
		HW.nadirma |= HalloweenFlags.NadirMa.SaidNo;
		...
		if (first) {
			...
			options.push({ nameStr : "Defy",
				...
				func() {
					...
					Text.NL();
					HalloweenScenes.NadirMaNoEntry(parse, func);
				}, enabled : true,
			});
			Gui.SetButtonsFromList(options, false, undefined);
		} else {
	...
	export function HarthonFirst() {
		...
		const first = !(HW.harthon & HalloweenFlags.Harthon.Met);
		HW.harthon |= HalloweenFlags.Harthon.Met;
		...
		options.push({ nameStr : "Flee!",
			...
			func() {
				...
				const success = Math.random() < (werewolf ? 0.75 : 0.5);
	...
	export function HarthonDefeatedPrompt() {
		...
		options.push({ nameStr : "Sex",
			...
			func() {
				...
				HW.harthon |= HalloweenFlags.Harthon.Thrall;
		...
		if (party.Inv().QueryNum(HalloweenItems.HolyWater) &&
			!(HW.harthon & HalloweenFlags.Harthon.Feminized)) {
			options.push({ nameStr : "Holy Water",
	...
	export function HarthonThrall() {
		...
		const first = !(HW.harthon & HalloweenFlags.Harthon.ThrallCalled);

		if (first) {
			HW.harthon |= HalloweenFlags.Harthon.ThrallCalled;
			...
			func() {
				...
				options.push({ nameStr : "Blowjob",
					...
					func() {
						...
						const first = !(HW.harthon & HalloweenFlags.Harthon.BJ);
						HW.harthon |= HalloweenFlags.Harthon.BJ;
	...
	HWLoc.Chapel.events.push(new Link(
		...
		() => {
			if (!(HW.flags & HalloweenFlags.Flags.Laggoth)) {
				Text.NL();
	...
	export function Laggoth() {
		...
		HW.flags |= HalloweenFlags.Flags.Laggoth;
	...
	export function Lenka() {
		...
		HW.flags |= HalloweenFlags.Flags.Lenka;
		...
		if (p1cock) {
			...
			const canine = player.Race().isRace(Race.Canine);
		...
		if (player.HasBalls()) {
			const size = player.Balls().BallSize();
			if (size >= 12) {
	...
	export function Jenna() {
		...
		const first = !(HW.flags & HalloweenFlags.Flags.Jenna);
		HW.flags |= HalloweenFlags.Flags.Jenna;
		...
		} else if (HW.flags & HalloweenFlags.Flags.Broomfuck) {
	...
	export function JennaBroomfuck() {
		...
		HW.flags |= HalloweenFlags.Flags.Broomfuck;
		...
		Text.NL();
		if (HW.flags & HalloweenFlags.Flags.Carepack) {
			...
			Text.NL();
			if (HW.flags & HalloweenFlags.Flags.PatchesPW) {
		...
		HW.flags |= HalloweenFlags.Flags.PatchesPW;
	...
	export function Patches() {
		...
		const success = () => {
			...
			Text.NL();
			parse.witch = HW.flags & HalloweenFlags.Flags.Jenna ? "Jenna" : "the witch";
			...
			HW.flags |= HalloweenFlags.Flags.Carepack;
		...
		options.push({ nameStr : "Input",
			...
			func() {
				...
				Gui.NextPrompt(() => {
					const truePw = Halloween.PW().toLowerCase();
					const pw     = textBox.value.toLowerCase();
