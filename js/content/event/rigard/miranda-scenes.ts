...
export namespace MirandaScenes {
	...
	export function RigardGatesDesc() {
	...
	let welcomeToRigardPASS: boolean;
	let welcomeToRigardOUTLAWS: boolean;
	...
	export function WelcomeToRigard() {
		...
		Gui.NextPrompt(() => {
			...
			welcomeToRigardPASS    = true;
			welcomeToRigardOUTLAWS = true;
			welcomeToRigardMIRANDA = true;
		...
		if (welcomeToRigardPASS) {
			options.push({ nameStr : "Pass",
				func() {
					...
					welcomeToRigardPASS = false;
					MirandaScenes.WelcomeToRigardQnA();
				}, enabled : true,
				tooltip : "Ask where one could acquire a pass.",
			});
		}
		if (welcomeToRigardOUTLAWS) {
	...
	*/
	...
	// TODO
	export function HomeSubbyDungeon() {
		const party: Party = GAME().party;
		const world = WORLD();
	...
	export function DatingFirstCity() {
		...
		if (rigard.Visa()) {
			...
			Text.NL();
			if (miranda.Attitude() >= MirandaFlags.Attitude.Neutral && datingScore >= 0) {
