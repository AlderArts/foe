...
import { Locations } from "../../engine/navigation/location";
...
export namespace RoamingScenes {
	...
	export function BanditsGen(capt: Entity, levelbonus: number) {
		const CreateBandit = () => {
		...
		enc.OnIncapacitate = function(entity: Entity) {
			...
			if (found) {
				for (const bandit of enemy.reserve) {
