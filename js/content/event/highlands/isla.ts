...
import { HipSize, LowerBodyType } from "../../../engine/entity/body/body";
import { Cock } from "../../../engine/entity/body/cock";
import { Color } from "../../../engine/entity/body/color";
import { Genitalia } from "../../../engine/entity/body/genitalia";
...
import { PregnancyHandler, Womb } from "../../../engine/entity/pregnancy";
...
export namespace IslaScenes {
	...
	export function Prompt() {
		...
		const womb = isla.PregHandler().Womb();
		const preg = womb && womb.pregnant;
		const stage = preg ? womb.progress : 0;
		...
		options.push({ nameStr : "Sex",
			...
			func() {
				if (isla.flags.Talk & IslaFlags.Talk.Sex) {
					IslaScenes.IslaSex.Repeat();
				} else {
					IslaScenes.IslaSex.First();
				}
			}, enabled : true,
		});
	...
	export namespace IslaSex {
		...
		export function PitchVaginal() {
			...
			IslaScenes.IslaSex.PitchVaginalPrompt(parse, {
				p1cock,
				strapon,
				knot,
				preg,
		...
		export function PitchVaginalPrompt(parse: IParse, opts: any) {
			...
			options.push({ nameStr : "Against Wall",
				...
				func() {
					...
					if (opts.stage >= 0.75) {
						...
						IslaScenes.IslaSex.PitchVaginalPrompt(parse, opts);
					} else {
						IslaScenes.IslaSex.PitchVaginalWall(parse, opts);
					}
				}, enabled : !opts.nowall,
			});
			options.push({ nameStr : "Mount Her",
				...
				func() {
					IslaScenes.IslaSex.PitchVaginalMount(parse, opts);
				}, enabled : true,
			});
		...
		export function PitchVaginalUnder(parse: IParse, opts: any) {
			...
			const cockSize = opts.p1cock.Volume();
	...
	export function BatheTF(parse: IParse, level: number) {
		...
		scenes.AddEnc(() => {
			...
			return true;
		}, 1.0, () => !player.body.head.race.isRace(Race.Ferret));
		scenes.AddEnc(() => {
			Text.NL();
			...
			return true;
		}, 1.0, () => { const tail = player.HasTail(); return !tail || !tail.race.isRace(Race.Ferret); });

		const IncompleteCockTf = () => {
			let changed = false;
		...
		scenes.AddEnc(() => {
			...
			const sheath = player.Genitalia().Sheath();
			...
			return true;
		}, 1.0, () => player.FirstCock() && IncompleteCockTf());
		scenes.AddEnc(() => {
			Text.NL();
			...
			return true;
		}, 1.0, () => player.body.muscleTone.Get() > 0);

		if (level >= IslaFlags.Bathing.Medium) {
			const IncompleteCockLenTf = () => {
				let changed = false;
				_.each(player.AllCocks(), (c) => {
			...
			scenes.AddEnc(() => {
				...
				return true;
			}, 1.0, () => player.HasBalls() && player.Balls().BallSize() < 6);
			scenes.AddEnc(() => {
				Text.NL();
				...
				return true;
			}, 1.0, () => player.FirstBreastRow().Size() < 12.5);
			scenes.AddEnc(() => {
				Text.NL();
				...
				return true;
			}, 1.0, () => player.FirstVag() && player.FirstVag().Capacity() < 8);
			scenes.AddEnc(() => {
				Text.NL();
				...
				return true;
			}, 1.0, () => player.body.torso.hipSize.Get() < HipSize.VeryWide || player.Butt().Size() < 9);
			scenes.AddEnc(() => {
				Text.NL();
				...
				return true;
			}, 1.0, () => player.body.femininity.Get() < 1);
			scenes.AddEnc(() => {
				Text.NL();
				...
				_.each(player.PregHandler().PregnantWombs(), (womb) => {
					const oldProgress = womb.progress;
					womb.progress += 0.15;
					const hours = 0.15 * womb.hoursToBirth / (1 - oldProgress);
					womb.hoursToBirth -= hours;
				});

				return true;
			}, 1.0, () => player.PregHandler().IsPregnant());
		}
		if (level >= IslaFlags.Bathing.Long) {
			...
			scenes.AddEnc(() => {
				...
				return true;
			}, 1.0, () => player.FirstCock() && !player.HasBalls());
			scenes.AddEnc(() => {
				Text.NL();
			...
			scenes.AddEnc(() => {
				...
				_.each(player.PregHandler().PregnantWombs(), (womb) => {
					const oldProgress = womb.progress;
					womb.progress += 0.3;
					const hours = 0.3 * womb.hoursToBirth / (1 - oldProgress);
					womb.hoursToBirth -= hours;
				});
