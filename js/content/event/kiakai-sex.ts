...
import { ColorDesc } from "../../engine/entity/body/color";
...
import { GAME, NAV, TimeStep } from "../../engine/GAME";
import { IngredientItems } from "../../engine/inventory/items/ingredients";
import { ItemToy } from "../../engine/inventory/items/toy-item";
...
export namespace KiakaiSexScenes {
	...
	export function Healing() {
		...
		if (player.HPLevel() < 0.25) { // REALLY LOW HEALTH 0-25%
			...
			KiakaiSexScenes.HealingSeducePrompt();
			...
			options.push({ nameStr : "Insist",
				func() {
					...
					KiakaiSexScenes.HealingAssertive();
				}, enabled : true,
	...
	export function HealingNice() {
			...
			options.push({ nameStr : "Eat you",
				func() {
					...
					Gui.Callstack.push(() => {
						...
						} else if (kiakai.flags.Attitude < KiakaiFlags.Attitude.Neutral) {
		...
		options.push({ nameStr : kiakai.name,
			func : KiakaiSexScenes.PleasureElf, enabled : true,
			...
			options.push({ nameStr : "Blowjob",
				func() {
					...
					if (kiakai.FirstCock().length.Get() > 35) {
						const len = player.FirstCock().length.Get();
	...
	export function HealingAssertive() {
		...
		let parse: IParse = {
			...
			name         : kiakai.name,
			heatStirring : player.FirstCock() ? "stirring" : "heat",
			tail() { const tail = player.HasTail(); return   tail ? tail.Short() : "NO TAIL"; },
		...
		if (player.FirstCock()) {
			options.push({ nameStr : "Blowjob",
				func() {
					...
					const biggestCockLen = biggestCock.length.Get();
			...
			func() {
				...
				options.push({ nameStr : "Anal",
					func() {
						...
						Text.NL();
						const eagerness = kiakai.flags.AnalExp * (1 + kiakai.LustLevel() * kiakai.Butt().Tightness());
						if (eagerness < 50) {
						...
						addToy(ToysItems.SmallDildo);
						addToy(ToysItems.MediumDildo);
						addToy(ToysItems.LargeDildo);
						addToy(ToysItems.ThinDildo);
						addToy(ToysItems.ButtPlug);
						addToy(ToysItems.LargeButtPlug);
						addToy(ToysItems.AnalBeads);
						addToy(ToysItems.LargeAnalBeads);
						addToy(ToysItems.EquineDildo);
		...
		if (kiakai.flags.TalkedSex === 0) {
			...
			options.push({ nameStr : "Love",
				func() {
					...
					Gui.NextPrompt(kiakai.Interact);
				}, enabled : kiakai.flags.Attitude > KiakaiFlags.Attitude.Neutral || kiakai.relation.Get() > 10,
			...
			options.push({ nameStr : "Fuck",
				func() {
					...
					Gui.NextPrompt(kiakai.Interact);
				}, enabled : kiakai.subDom.Get() < -25 || kiakai.flags.Attitude < KiakaiFlags.Attitude.Neutral,
			...
			options.push({ nameStr : "Passive",
				func() {
					...
					KiakaiSexScenes.SexPrompt(KiakaiFlags.SexFirstAttitude.Passive);
				}, enabled : true,
			...
			options.push({ nameStr : "Mutual",
				func() {
					...
					kiakai.flags.TalkedSex = 3;
					KiakaiSexScenes.SexPrompt(KiakaiFlags.SexFirstAttitude.Mutual);
				}, enabled : true,
			...
			options.push({ nameStr : "Dig in",
				func() {
					...
					kiakai.flags.TalkedSex = 3;
					KiakaiSexScenes.SexPrompt(KiakaiFlags.SexFirstAttitude.Assertive);
				}, enabled : true,
			...
			const domStat = player.subDom.Get() - kiakai.subDom.Get();
			if (domStat > 50) {
	...
	export function SexPrompt(attitude?: number) {
		...
		let parse: IParse = {
			...
			kCockDesc2() { return kiakai.AllCocks()[1].Short(); },
			boygirl      : kiakai.body.femininity.Get() > 0 ? "girl" : "boy",
			cockType() { return strapon ? "strap-on" : playerCock.noun(); },
		};
		...
		options.push({ nameStr : "Anal pitch",
			func() {
				...
				const len = playerCock.Len();
				const thk = playerCock.Thickness();
			...
			func() {
				...
				scenes.AddEnc(() => {
					...
					scenes.AddEnc(() => {
						...
						KiakaiSexScenes.AnalCatchPrep(KiakaiFlags.AnalCatchPrepScene.Rim);
					}, 1.0, () => true);
					scenes.AddEnc(() => {
						Text.Add("<i>“C-can you suck on it?”</i> The elf bites [hisher] lip, uncertain on how you will respond.", parse);
						KiakaiSexScenes.AnalCatchPrep(KiakaiFlags.AnalCatchPrepScene.Suck);
					}, 1.0, () => true);
					scenes.AddEnc(() => {
						...
						KiakaiSexScenes.AnalCatchPrep(KiakaiFlags.AnalCatchPrepScene.Selfsuck);
				...
				scenes.AddEnc(() => {
					...
					const scenes = new EncounterTable();
					scenes.AddEnc(() => {
						KiakaiSexScenes.AnalCatchPrep(KiakaiFlags.AnalCatchPrepScene.Suck, true);
					}, 1.0, () => true);
					scenes.AddEnc(() => {
	...
	export function AnalCatchPrep(choice?: number, assert?: boolean) {
		...
		options.push({ nameStr : "Rim",
			func() {
				KiakaiSexScenes.AnalCatchPrep(KiakaiFlags.AnalCatchPrepScene.Rim, true);
			}, enabled : true,
		...
		options.push({ nameStr : "Selfsuck",
			func() {
				KiakaiSexScenes.AnalCatchPrep(KiakaiFlags.AnalCatchPrepScene.Selfsuck, true);
		...
		options.push({ nameStr : "Oil",
			func() {
				KiakaiSexScenes.AnalCatchPrep(KiakaiFlags.AnalCatchPrepScene.Oil, true);
			}, enabled : party.inventory.QueryNum(IngredientItems.SnakeOil),
		...
		if (!choice) {
			...
			options.push({ nameStr : "Yes",
				func() {
					kiakai.subDom.IncreaseStat(10, 1);
					KiakaiSexScenes.AnalCatchPrep(choice, true);
				}, enabled : true,
		...
		options.push({ nameStr : "Take charge",
			func() {
				...
				const expFraction = 0.5 * player.sexlevel / kiakai.sexlevel;
				// Elf cums first
					...
					if (kiakai.FirstCock()) {
						...
						options.push({ nameStr : "Inside",
							func() {
								...
								KiakaiSexScenes.AnalCatchFinish();
							}, enabled : true,
