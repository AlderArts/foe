...
import { GAME, MoveToLocation, NAV, TimeStep, WORLD, WorldTime } from "../../../../engine/GAME";
...
import { ILocRigardKrawitz } from "../../../../engine/navigation/location";
...
export namespace KrawitzScenes {
	...
	export function SetupStats() {
		...
		stat.IsServant         = rigard.Krawitz.Work === 2;
		stat.HasServantClothes = stat.IsServant;
		stat.HasWine           = false;
		stat.LustPotion        = false;
		stat.ServantFood       = 0;
		stat.KrawitzFood       = 0;
		stat.ChestLocKnown     = false;
		stat.TFItem            = false;
		stat.TFdKrawitz        = false;
		stat.BathhouseVisit    = false;
		stat.BathhouseWine     = false;
		stat.BathhouseSpiked   = false;
		stat.SexedGirls        = false;
		stat.MansionVisit      = false;
		stat.KitchenVisit      = false;
		stat.ServantWine       = false;
		stat.ServantSpikedWine = false;
		stat.ServSantOrgySetup = false;
		stat.Orgy              = false;
		stat.HasSword          = false;
		stat.HasBinder         = false;

		stat.Swiped            = false;

		stat.Suspicion         = 0;
		stat.AlarmRaised       = false;
		stat.guardRot          = 0;
		stat.servantRot        = 0;
	...
	export function GuardDex(entity: EncType, num?: number) {
		num = num || 1;
		const dex = (entity === EncType.Guard) ? 10 : 15;
		return num * dex;
	}
	export function GuardAtk(entity: EncType, num?: number) {
		num = num || 1;
		const str = (entity === EncType.Guard) ? 40 : 20;
		return num * str;
	}
	export function GuardCha(entity: EncType, num?: number) {
		num = num || 1;
		let cha = (entity === EncType.Guard) ? 15 : 10;
		if (entity === EncType.Guard) {
			if (stat.HasServantClothes) { cha /= 2; }
		} else { // Servants
			if (stat.IsServant) { cha /= 4; } else if (stat.HasServantClothes) { cha *= 1.5; }
		}

		return num * cha;
	}
	export function ServantSuspicion() {
		if (stat.IsServant) { return 1; } else if (stat.HasServantClothes) { return 3; } else { return 2; }
	}
	export function GuardSuspicion() {
		if (stat.HasServantClothes) { return 1; } else { return 3; }
	}
	export function EntitySuspicion(entity: EncType) {
		if (entity === EncType.Guard) {
	...
	KrawitzLoc.Street.links.push(new Link(
		"Grounds", true, () => WorldTime().hour >= 20,
		() => {
	...
	KrawitzLoc.Grounds.enc.AddEnc(() => KrawitzScenes.PatrollingGuards, 1.0, () => !stat.Orgy);
	KrawitzLoc.Grounds.enc.AddEnc(() => KrawitzScenes.WanderingServants, 1.0, () => !stat.ServantSpikedWine);

	KrawitzLoc.Grounds.description = () => {
	...
	KrawitzLoc.Grounds.onEntry = () => {
		const enc = new EncounterTable();
		enc.AddEnc(KrawitzScenes.PatrollingGuards,  1.0, () => !stat.Orgy);
		enc.AddEnc(KrawitzScenes.WanderingServants, 1.0, () => !stat.ServantSpikedWine);
	...
	KrawitzLoc.Grounds.links.push(new Link(
		...
		() => {
			...
			const ServantPrompt = () => {
				...
				if (stat.ServantOrgySetup) {
				...
				} else if (stat.ServantSpikedWine) {
					...
					if (stat.HasWine && !stat.ServantWine) {
					...
					if (stat.LustPotion && stat.HasWine && !stat.ServantSpikedWine) {
	...
	// Mansion: Hall
	//
	KrawitzLoc.Mansion.Hall.description = () => {
		if (Math.random() < 0.3) {
	...
	KrawitzLoc.Mansion.Hall.links.push(new Link(
		...
		() => {
			...
			if (stat.ServantFood === 1) {
				...
				Text.NL();
				stat.KrawitzFood = 2;
			} else if (stat.BathhouseVisit && !stat.HasWine) {
		...
		() => {
			...
			if (stat.KrawitzFood === 0) {
				...
				if (stat.HasServantClothes) {
					...
					const options = KrawitzScenes.KrawitzPrompt();
					Gui.SetButtonsFromList(options);
				} else {
					KrawitzScenes.FightKrawitz();
				}
			} else {
				if (stat.KrawitzFood === 3) {
	...
	// Mansion: Study
	//
	KrawitzLoc.Mansion.Study.description = () => {
	...
	export function EnteringTheWork() {
		const rigard = GAME().rigard;
		const party: Party = GAME().party;
	...
	export function FoundOut(entity: EncType, num: number, gender?: Gender) {
		...
		if (stat.HasWine) {
			options.push({ nameStr : stat.LustPotion && stat.HasWine ? "Spiked Wine" : "Wine",
				func() {
					let s = KrawitzScenes.EntitySuspicion(entity);
					Text.Clear();
					...
					if (stat.LustPotion && stat.HasWine) {
						Text.NL();
	...
	export function AddSuspicion(num: number, surpressNext?: boolean) {
		...
		if (GetDEBUG()) {
			Text.NL();
			Text.Add("<b>Suspicion + " + num + ": " + stat.Suspicion + "/100</b>");
			Text.NL();
			Text.Flush();
		}

		if (stat.Suspicion >= 100) {
			stat.AlarmRaised = true;
	...
	export function PatrollingGuards() {
		...
		if (rand < 0.2) {
			...
			Text.Flush();
			Gui.NextPrompt();
		} else if (stat.HasServantClothes && rand < 0.5) {
	...
	export function WanderingServants() {
		...
		if (rand < 0.4) {
			...
			Text.Flush();
			Gui.NextPrompt();
		} else if ((stat.IsServant && rand < 0.8) || (stat.HasServantClothes && rand < 0.3)) {
	...
	export function Bathhouse() {
		...
		Text.Clear();
		if (!stat.BathhouseVisit) {
			stat.BathhouseVisit = true;
			...
			if (stat.HasWine && stat.LustPotion) {
				options.push({ nameStr : "Spiked Wine",
					func() {
						Text.Clear();
						KrawitzScenes.BathhouseWine();
						stat.BathhouseSpiked = true;
						Text.NL();
						...
						options.push({ nameStr : "Sex",
							func() {
								...
								Gui.NextPrompt(() => {
									const playerCock = player.FirstCock() || (player.strapOn ? player.strapOn.cock : StrapOnItems.LargeStrapon.cock);
	...
	export function Aftermath() {
		...
		let points = 0;
		if     (rigard.Krawitz.Duel === 1) {    points += 2; } else if (rigard.Krawitz.Duel === 2) {    points += 1; }
		if (stat.HasSword) {        points += 2; }
		if (stat.HasBinder) {       points += 1; }
		if (stat.TFdKrawitz) {      points += 2; }
		if (stat.BathhouseSpiked) { points += 1; }
		if (stat.Orgy) {            points += 2; }
		...
		// Save flags
		rigard.Krawitz.F = 0;
		if (stat.HasServantClothes) { rigard.Krawitz.F |= KrawitzFlags.Clothes; }
		if (stat.HasBinder) { rigard.Krawitz.F |= KrawitzFlags.Binder; }
		if (stat.HasSword) { rigard.Krawitz.F |= KrawitzFlags.Sword; }
		if (stat.BathhouseSpiked) { rigard.Krawitz.F |= KrawitzFlags.SpikedLadies; }
		if (stat.SexedGirls) { rigard.Krawitz.F |= KrawitzFlags.Sex; }
		if (stat.ServantSpikedWine) { rigard.Krawitz.F |= KrawitzFlags.SpikedServants; }
		if (stat.Orgy) { rigard.Krawitz.F |= KrawitzFlags.Orgy; }
		if (stat.TFdKrawitz) { rigard.Krawitz.F |= KrawitzFlags.TF; }

		Text.Clear();
		Text.Add("<b>Final Score:</b><br>", parse);
		Text.Add("Suspicion raised: " + stat.Suspicion + "/100<br>", parse);
		Text.Add("Mayhem spread: " + points + "/10<br>", parse);
		Text.Add("Alarm raised: " + (stat.AlarmRaised ? "yes" : "no"), parse);
		Text.Flush();

		if (!stat.AlarmRaised) { points += 1; }
	...
	let twinsTalkRoyals: boolean;
	let twinsTalkKrawitz: boolean;
	let twinsTalkDisguise: boolean;
	...
	export function TwinsTalk() {
		...
		twinsTalkRoyals   = false;
		twinsTalkKrawitz  = false;
		twinsTalkDisguise = false;
		twinsTalkLei      = false;
		...
		if (!twinsTalkKrawitz) {
			options.push({ nameStr : "Krawitz",
				func() {
					...
					Text.Flush();
					twinsTalkKrawitz = true;
					KrawitzScenes.TwinsPrompt();
				}, enabled : true,
				tooltip : "What is their gripe with Krawitz?",
			});
		}
		if (!twinsTalkDisguise) {
