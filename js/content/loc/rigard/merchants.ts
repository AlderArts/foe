...
import { ILocRigardShops } from "../../../engine/navigation/location";
...
import { Room69 } from "../../event/rigard/room69";
import { Room69Flags } from "../../event/rigard/room69-flags";
...
// Shopping street
//
ShopStreetLoc.Street.description = () => {
...
ShopStreetLoc.Street.enc.AddEnc(() => RigardScenes.Chatter);
ShopStreetLoc.Street.enc.AddEnc(() => RigardScenes.Chatter2);
ShopStreetLoc.Street.enc.AddEnc(() => ShopStreetScenes.Speculate, 1.0, () => (WorldTime().hour >= 6 && WorldTime().hour < 18));
ShopStreetLoc.Street.enc.AddEnc(() => RigardScenes.CityHistory, 1.0, () => {
	const rigard = GAME().rigard;
	return rigard.flags.CityHistory === 0;
});
ShopStreetLoc.Street.enc.AddEnc(() => TerryScenes.ExploreMerchants, 1000000.0, () => {
	const rigard = GAME().rigard;
	return rigard.Krawitz.Q === RigardFlags.KrawitzQ.HuntingTerry;
});
ShopStreetLoc.Street.onEntry = () => {
	if (Math.random() < 0.15) {
...
ShopStreetLoc.Street.links.push(new Link(
	"Armor", true, () => ArmorShopScenes.IsOpen(),
	() => {
...
ShopStreetLoc.Street.links.push(new Link(
	"Weapons", true, () => WeaponShopScenes.IsOpen(),
	() => {
...
ShopStreetLoc.Street.links.push(new Link(
	"Tailor", true, () => ClothShopScenes.IsOpen(),
	() => {
...
ShopStreetLoc.Street.links.push(new Link(
	"Magic", true, () => MagicShopScenes.IsOpen(),
	() => {
...
ShopStreetLoc.Street.links.push(new Link(
	"Odd shop", true, () => OddShopScenes.IsOpen(),
	() => {
