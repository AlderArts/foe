...
export namespace NagaScenes {

	export function Impregnate(mother: Entity, father: Naga, slot?: number) {
		...
		if (nagaMate) {
			...
			options.push({ nameStr : "Fuck",
				func() {
					...
					const ret = NagaScenes.DesertLossScenes(enc);
	...
	export function DesertWinFuckJerk(enc: NagaEncounter, hypno: boolean) {
		...
		if (player.HasBalls()) {
			const sacksize = player.Balls().SackSize();
	...
	export function DesertWinGetFuckedVag(enc: NagaEncounter, hypno: boolean) {
		...
		NagaScenes.DesertWinTailpeg({naga, hypno,
			next(tailPeg: boolean) {
				let sens = 0;
				if (tailPeg) { sens++; }
