...
import { Race, RaceDesc } from "../../engine/entity/body/race";
...
export namespace FelinesScenes {

	export function WildcatEnc(levelbonus: number) {
	...
	export function PumaEnc(levelbonus: number) {
		const enemy = new Party();
		const r = Math.random();
		if (r < 0.2) {
	...
	export function JaguarEnc(levelbonus: number) {
		const enemy = new Party();
		const r = Math.random();
		if (r < 0.2) {
	...
	export function LynxEnc(levelbonus: number) {
		const enemy = new Party();
		const r = Math.random();
		if (r < 0.2) {
	...
	export function Impregnate(mother: Entity, father: Wildcat, slot?: number) {
	...
	export function LossPitchVaginal(cat: Wildcat, group: boolean, enc: Encounter, cocksInVag: Cock[]) {
		...
		const options: IChoice[] = [];
		options.push({ nameStr : "Pound Her",
			func() {
				Text.Clear();
				if (dom) {
		...
		options.push({ nameStr : "Titty Fuck",
			func() {
				Text.Clear();
				if (dom) {
